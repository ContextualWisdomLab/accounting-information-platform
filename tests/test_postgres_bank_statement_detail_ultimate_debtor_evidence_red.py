"""PostgreSQL REDs for transaction-detail ultimate-debtor evidence."""
...
class BankStatementDetailUltimateDebtorEvidenceRedTests(unittest.TestCase):
    """Retain bank-reported ultimate-debtor evidence at transaction-detail scope."""
    ...
    def setUp(self) -> None:
        """Create statements differing only in one ultimate-debtor name."""
    ...
    def test_ultimate_debtor_change_changes_detail_entry_and_statement_hashes(self) -> None:
    ...
    def test_entry_lookup_preserves_ultimate_debtor_evidence_hash(self) -> None:
        """Buyer-visible detail reads retain purpose-bound ultimate-debtor evidence."""
        ...
        self.assertEqual(first_detail["ultimate_debtor_evidence_hash"], expected_hash)

    @staticmethod
    def _with_ultimate_debtor(fixture: str, marker: str, value: str) -> bytes:
        """Insert one schema-shaped ultimate debtor after the existing debtor."""
        replacement = (
            "              </Dbtr>\n"
            "              <UltmtDbtr>\n"
            "                <Pty>\n"
    ...
    def _command(self, payload: bytes, suffix: str) -> dict[str, object]:
        ...
        return {
            "tenant_reference": self.case.policy.tenant_reference,
            "bank_account_reference": self.bank_account_reference,
            "ingestion_idempotency_key": f"ultimate-debtor-{suffix}-{uuid.uuid4().hex}",
