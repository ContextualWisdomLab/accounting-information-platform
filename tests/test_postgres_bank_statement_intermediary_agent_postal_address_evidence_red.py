"""PostgreSQL REDs for intermediary-agent PostalAddress27 evidence preservation."""
...
_INSTITUTION_ADDRESS = {
    "street_name": "Intermediary Institution Street",
    "building_number": "10",
    "post_code": "10000",
    "town_name": "Intermediary City",
    "country": "US",
    "address_lines": [
        "Intermediary Institution Street 10",
        "10000 Intermediary City",
    ],
}

_BRANCH_ADDRESS = {
    "street_name": "Intermediary Branch Street",
    "building_number": "20",
    "post_code": "20000",
    "town_name": "Intermediary Branch City",
    "country": "US",
    "address_lines": [
        "Intermediary Branch Street 20",
        "20000 Intermediary Branch City",
    ],
}


class BankStatementIntermediaryAgentPostalAddressEvidenceRedTests(unittest.TestCase):
    """Retain institution and branch PostalAddress27 for all intermediary positions."""
    ...
    def test_postal_address_is_material_at_each_chain_position(self) -> None:
        """Institution and branch postal facts change canonical intermediary evidence."""
        for slot in (1, 2, 3):
    ...
    def test_postal_layout_is_representation_only_at_each_chain_position(self) -> None:
        """Whitespace in institution or branch PstlAdr changes bytes, not semantics."""
        for slot in (1, 2, 3):
            ...
            for placement, needle in (
                ("institution", b"                  <PstlAdr>\n"),
                ("branch", b"                  <PstlAdr>\n"),
            ):
                with self.subTest(slot=slot, placement=placement):
                    occurrences = [
                        index
                    ...
                    target_index = occurrences[0 if placement == "institution" else 1]
                    changed_payload = (
    ...
    def test_postal_change_requires_explicit_correction(self) -> None:
        """Accepted intermediary postal evidence cannot be replaced on replay."""
        slot = 3
        base_payload = self._payload(
            slot,
            institution_address=_INSTITUTION_ADDRESS,
        ...
        changed_address["street_name"] = "Changed Intermediary Branch Street"
    ...
    def test_postal_evidence_stays_digest_only_on_buyer_projection(self) -> None:
        """Buyer reads expose only the purpose digest for intermediary postal identity."""
        for slot in (1, 2, 3):
            with self.subTest(slot=slot):
                ...
                rich_detail = self.case._ingest_and_read(
                    rich_payload,
                    rich_reference,
                    f"postal-rich-{slot}-{uuid.uuid4().hex}",
                )
                no_postal_detail = self.case._ingest_and_read(
                    no_postal_payload,
                    no_postal_reference,
                    f"postal-none-{slot}-{uuid.uuid4().hex}",
                )
                digest_key = f"intermediary_agent_{slot}_evidence_hash"
    ...
    def _variants(self, slot: int) -> dict[str, bytes]:
        """Create one-field postal changes and whole-address absence variants."""
        institution_street = copy.deepcopy(_INSTITUTION_ADDRESS)
        institution_street["street_name"] = "Alternate Intermediary Institution Street"
        institution_line = copy.deepcopy(_INSTITUTION_ADDRESS)
        institution_line["address_lines"][1] = "10001 Alternate Intermediary City"
        branch_street = copy.deepcopy(_BRANCH_ADDRESS)
        branch_street["street_name"] = "Alternate Intermediary Branch Street"
    ...
    def _payload(
        ...
        *,
        ...
        """Insert institution and branch PostalAddress27 into the target intermediary."""
        ...
        if branch_address is not None:
            ...
            if payload.count(branch_needle) != 1:
                raise AssertionError("target intermediary branch name must be unique")
            payload = payload.replace(
                branch_needle,
