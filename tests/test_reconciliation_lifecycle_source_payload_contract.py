...
class ReconciliationLifecycleSourcePayloadContractTests(unittest.TestCase):
    """Keep strict JSON identity and migration 0028 on the supported boundary."""
    ...
    def test_install_fails_closed_when_source_identity_migration_is_missing(self) -> None:
        ...
        def is_file(path: Path) -> bool:
            if path.name == "0028_reconciliation_lifecycle_source_payload_identity.sql":
                return False
        ...
        with patch.object(Path, "is_file", is_file):
            with self.assertRaisesRegex(
                ...
                "0028_reconciliation_lifecycle_source_payload_identity",
            ):
