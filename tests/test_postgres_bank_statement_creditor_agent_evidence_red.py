"""PostgreSQL REDs for camt.053 creditor-agent evidence preservation."""
...
class BankStatementCreditorAgentEvidenceRedTests(unittest.TestCase):
    """Retain the reported creditor agent at transaction-detail evidence granularity."""
    ...
    def setUp(self) -> None:
        """Create statements differing only in the first detail's creditor-agent BICFI."""
        ...
        self.first_creditor_agent_bicfi = "BNPAFRPP"
        self.second_creditor_agent_bicfi = "SOGEFRPP"
        self.first_payload = self._with_creditor_agent(
            fixture,
            marker,
            self.first_creditor_agent_bicfi,
        )
        self.second_payload = self._with_creditor_agent(
            fixture,
            marker,
            self.second_creditor_agent_bicfi,
        )
    ...
    def test_creditor_agent_change_changes_detail_entry_and_statement_hashes(self) -> None:
        """Changing only RltdAgts/CdtrAgt BICFI changes all retained evidence identities."""
    ...
    def test_same_statement_identity_cannot_replay_changed_creditor_agent(self) -> None:
        """Changed reported creditor agent reaches the statement correction boundary."""
        accept_bank_statement_evidence(
    ...
    def test_entry_lookup_preserves_creditor_agent_evidence_hash(self) -> None:
        """Buyer-visible detail reads retain purpose-bound creditor-agent evidence."""
        ...
        self.assertEqual(detail["creditor_agent_evidence_hash"], expected_hash)

    def test_creditor_agent_projection_differs_from_baseline_only_by_digest(self) -> None:
        """Creditor-agent source evidence adds no reversible buyer projection field."""
        ...
        baseline_projection.pop("creditor_agent_evidence_hash", None)
    ...
    @staticmethod
    def _with_creditor_agent(fixture: str, marker: str, bicfi: str) -> bytes:
        """Insert one reported creditor agent beside the existing related parties."""
        replacement = (
            "            </RltdPties>\n"
            "            <RltdAgts>\n"
            "              <CdtrAgt>\n"
        ...
        accepted = accept_bank_statement_evidence(
            {
                ...
                "ingestion_idempotency_key": (
                    f"creditor-agent-{suffix}-{uuid.uuid4().hex}"
                ),
    ...
    def _creditor_agent_evidence_hash(self) -> str:
        """Return the purpose-bound digest expected by the buyer projection."""
        return "sha256:" + hashlib.sha256(
            self.first_creditor_agent_bicfi.encode("utf-8")
        ).hexdigest()
    ...
    def _command(self, payload: bytes, suffix: str) -> dict[str, object]:
        ...
        return {
            "tenant_reference": self.case.policy.tenant_reference,
            "bank_account_reference": self.bank_account_reference,
            "ingestion_idempotency_key": f"creditor-agent-{suffix}-{uuid.uuid4().hex}",
            "message_definition_identifier": CAMT053_MESSAGE_DEFINITION,
            "statement_payload": payload.decode("utf-8"),
        }
