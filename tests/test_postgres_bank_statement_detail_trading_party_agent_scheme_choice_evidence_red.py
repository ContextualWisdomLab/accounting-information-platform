"""PostgreSQL REDs for trading-agent scheme choice and optionality evidence."""
...
class BankStatementDetailTradingPartyAgentSchemeChoiceEvidenceRedTests(
    unittest.TestCase
):
    """Retain scheme choice semantics without promoting them to accounting truth."""
    ...
    def setUp(self) -> None:
        """Prepare clearing-system and alternate-ID scheme choice variants."""
        ...
        self.variants = {
            "clearing-system-coded-choice": copy.deepcopy(self.base),
            "clearing-system-absent": copy.deepcopy(self.base),
            "other-scheme-coded-choice": copy.deepcopy(self.base),
            "other-scheme-absent": copy.deepcopy(self.base),
        }
        self.variants["clearing-system-coded-choice"]["financial_institution"][
            "clearing_system_member"
        ]["clearing_system"] = {"code": "DEBLZ"}
        self.variants["clearing-system-absent"]["financial_institution"][
            "clearing_system_member"
        ].pop("clearing_system")
        self.variants["other-scheme-coded-choice"]["financial_institution"]["other"][
            "scheme"
        ] = {"code": "BANK"}
        self.variants["other-scheme-absent"]["financial_institution"]["other"].pop(
            "scheme"
        )
    ...
    def test_xml_layout_does_not_change_scheme_choice_identity(self) -> None:
        """Whitespace inside ClrSysId changes bytes, not semantic identity."""
    ...
    def test_changed_scheme_choice_requires_explicit_correction(self) -> None:
        """Accepted choice provenance cannot be replaced by silent replay."""
    ...
    def _assert_exact_accounting_amount(entry: object, detail: object) -> None:
        """Pin accounting truth independently from bank-reported scheme choices."""
        if getattr(entry, "entry_amount", None) != Decimal("25000.00"):
            raise AssertionError("scheme choices must retain exact 25000.00 entry amount")
        if getattr(entry, "entry_currency_code", None) != "KRW":
            raise AssertionError("scheme choices must retain KRW entry currency")
        if getattr(detail, "detail_amount", None) != Decimal("25000.00"):
            raise AssertionError("scheme choices must retain exact 25000.00 detail amount")
        if getattr(detail, "detail_currency_code", None) != "KRW":
            raise AssertionError("scheme choices must retain KRW detail currency")
    ...
    @staticmethod
    def _choice_xml(value: dict[str, object], indent: str) -> str:
        """Serialize exactly one Cd or Prtry branch."""
        if set(value) == {"code"}:
            return f"{indent}<Cd>{value['code']}</Cd>\n"
        if set(value) == {"proprietary"}:
            return f"{indent}<Prtry>{value['proprietary']}</Prtry>\n"
        raise AssertionError("scheme choice must contain exactly one of code/proprietary")

    @classmethod
    def _trading_party_xml(cls, value: dict[str, object]) -> str:
        """Serialize clearing and alternate-ID choice evidence."""
        if value.get("choice") != "agent":
            raise AssertionError("scheme-choice RED requires the Agt branch")
        ...
        if clearing_system is not None:
            if not isinstance(clearing_system, dict):
                raise AssertionError("clearing-system choice must be structured")
            clearing_system_xml = (
                "                      <ClrSysId>\n"
                + cls._choice_xml(clearing_system, "                        ")
                + "                      </ClrSysId>\n"
            )
        ...
        if scheme is not None:
            if not isinstance(scheme, dict):
                raise AssertionError("financial-ID scheme choice must be structured")
            scheme_xml = (
                "                      <SchmeNm>\n"
                + cls._choice_xml(scheme, "                        ")
                + "                      </SchmeNm>\n"
            )
    ...
    def _with_trading_party(
        ...
        """Insert complete scheme-choice evidence after debtor evidence."""
    ...
    def _command(self, payload: bytes, suffix: str) -> dict[str, object]:
        ...
        return {
            ...
            "ingestion_idempotency_key": (
                f"detail-trading-agent-scheme-choice-{suffix}-{uuid.uuid4().hex}"
            ),
