"""PostgreSQL REDs for debtor/creditor-agent AddressType3Choice evidence."""
...
from tests import (
    test_postgres_bank_statement_debtor_creditor_agent_postal_address_evidence_red as postal,
)
...
class BankStatementDebtorCreditorAgentAddressTypeChoiceEvidenceRedTests(
    unittest.TestCase
):
    """Retain institution and branch AddressType3Choice for DbtrAgt and CdtrAgt."""

    @classmethod
    def setUpClass(cls) -> None:
        """Reuse the source-real PostgreSQL and camt.053 integration fixture."""
        postal.BankStatementDebtorCreditorAgentPostalAddressEvidenceRedTests.setUpClass()

    def setUp(self) -> None:
        """Prepare the reviewed postal helper with exception-safe nested cleanup."""
        self.case = postal.BankStatementDebtorCreditorAgentPostalAddressEvidenceRedTests(
            "setUp"
        )
    ...
    def test_address_type_choice_is_material_for_both_roles_and_owners(self) -> None:
        """Cd/Prtry and proprietary fields change canonical role evidence."""
        for role, identity in self.case.case.AGENTS.items():
            for owner in ("institution", "branch"):
                with self.subTest(role=role, owner=owner):
                    plain = self._plain(role)
    ...
    def test_proprietary_choice_layout_is_representation_only(self) -> None:
        ...
        for role in self.case.case.AGENTS:
            for owner in ("institution", "branch"):
                with self.subTest(role=role, owner=owner):
                    ...
                    digest_key = self.case.case.AGENTS[role]["digest_key"]
                    base_digest = getattr(base_detail, digest_key, None)
    ...
    def test_changed_choice_requires_explicit_correction(self) -> None:
        """Accepted debtor/creditor address-type evidence cannot change on replay."""
        role = "CdtrAgt"
    ...
    def test_choice_evidence_stays_digest_only_on_buyer_projection(self) -> None:
        """Buyer reads expose no reversible debtor/creditor address-type fields."""
        for role, identity in self.case.case.AGENTS.items():
            for owner in ("institution", "branch"):
                with self.subTest(role=role, owner=owner):
                    plain = self._plain(role)
                    ...
                    proprietary_reference = self.case.case._register_account(
                        ...
                        f"address-type-proprietary-{role.lower()}-{owner}",
                    )
                    coded_reference = self.case.case._register_account(
                        coded_statement,
                        f"address-type-coded-{role.lower()}-{owner}",
                    )
    ...
    def _plain(self, role: str) -> bytes:
        """Return the postal-rich target role without AddressType3Choice."""
    ...
    def _with_choice(payload: bytes, owner: str, choice: bytes) -> bytes:
        ...
        if len(offsets) != 2:
            raise AssertionError(
                "focused debtor/creditor agent must contain two postal addresses"
            )
