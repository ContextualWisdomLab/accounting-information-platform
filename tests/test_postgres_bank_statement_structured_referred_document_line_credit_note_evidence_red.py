"""REDs for structured referred-document line-level credit-note evidence preservation."""
...
class BankStatementStructuredLineCreditNoteEvidenceRedTests(unittest.TestCase):
    """Retain LineDtls/Amt/CdtNoteAmt values with their exact source line."""
    ...
    def setUp(self) -> None:
        """Prepare line-credit-note statements differing in one later amount."""
        ...
        self.first_line_credit_note_amount = "1200"
        self.base_second_line_credit_note_amount = "800"
        self.changed_second_line_credit_note_amount = "801"
        ...
        self.base_payload = self._with_line_credit_notes(
            line_payload,
            self.base_second_line_credit_note_amount,
        )
        self.changed_payload = self._with_line_credit_notes(
            line_payload,
            self.changed_second_line_credit_note_amount,
        )
        ...
        self.bank_account_reference = (
            f"urn:cwl:bank_account:structured-line-credit-note:{uuid.uuid4().hex}"
        )
        accept_bank_account_record(
            {
    ...
    def test_later_line_credit_note_is_material_to_each_canonical_evidence_hash(self) -> None:
        """Bind one later credit-note-only change to the complete evidence chain."""
        changed_marker = (
            f'<CdtNoteAmt Ccy="KRW">{self.changed_second_line_credit_note_amount}'
            "</CdtNoteAmt>"
        )
        base_marker = (
            f'<CdtNoteAmt Ccy="KRW">{self.base_second_line_credit_note_amount}'
            "</CdtNoteAmt>"
        )
        changed_text = self.changed_payload.decode("utf-8")
        if changed_text.count(changed_marker) != 1:
            raise AssertionError("changed line credit-note marker must occur exactly once")
        self.assertEqual(
    ...
    def test_changed_line_credit_note_requires_explicit_statement_correction(self) -> None:
        """Credit-note-only source changes cannot silently replay one statement identity."""
        accepted = accept_bank_statement_evidence(
            self._command(self.base_payload, "line-credit-note-base"),
            posting.DATABASE_URL,
            self.case.policy.tenant_reference,
        ...
        with self.assertRaisesRegex(AccountingValidationError, _CORRECTION_ERROR):
            accept_bank_statement_evidence(
                self._command(self.changed_payload, "line-credit-note-changed"),
                posting.DATABASE_URL,
                self.case.policy.tenant_reference,
                artifact_store=self.store,
            )

    def test_buyer_read_keeps_credit_note_amount_bound_to_each_line(self) -> None:
        """Buyer reads retain each credit-note amount inside its exact source line."""
        accepted = accept_bank_statement_evidence(
            self._command(self.changed_payload, "line-credit-note-lookup"),
            posting.DATABASE_URL,
            self.case.policy.tenant_reference,
            artifact_store=self.store,
        )
        ...
        self.assertEqual(
            ...
            self._structured_projection(self.changed_second_line_credit_note_amount),
        )
    ...
    def _structured_projection(
        self,
        second_line_credit_note_amount: str,
    ) -> list[dict[str, object]]:
        """Extend the canonical line projection with line-bound credit-note evidence."""
        ...
        line_details[0]["credit_note_amount"] = {
            "amount": self.line_contract._decimal_text(
                Decimal(self.first_line_credit_note_amount)
            ),
            "currency_code": "KRW",
        }
        line_details[1]["credit_note_amount"] = {
            "amount": self.line_contract._decimal_text(
                Decimal(second_line_credit_note_amount)
            ),
            "currency_code": "KRW",
        }
        return projection

    def _with_line_credit_notes(
        self,
        line_payload: bytes,
        second_line_credit_note_amount: str,
    ) -> bytes:
        """Insert source-ordered line credit notes after discounts and before remittance."""
        ...
        first_credit_note = (
            ...
            f"{self.first_line_credit_note_amount}</CdtNoteAmt>\n"
            f"{first_marker}"
        )
        second_credit_note = (
            f'                      <CdtNoteAmt Ccy="KRW">'
            f"{second_line_credit_note_amount}</CdtNoteAmt>\n"
            f"{second_marker}"
        )
    ...
    def _command(self, payload: bytes, suffix: str) -> dict[str, object]:
        ...
        return {
            ...
            "ingestion_idempotency_key": (
                f"structured-line-credit-note-{suffix}-{uuid.uuid4().hex}"
            ),
