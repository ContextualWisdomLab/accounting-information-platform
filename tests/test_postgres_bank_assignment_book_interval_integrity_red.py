...
class BankAssignmentBookIntervalIntegrityRedTests(unittest.TestCase):
    ...
    def test_assignment_end_cannot_extend_beyond_existing_book_end(self) -> None:
        """Extending an assignment end may not make it outlive its Accounting Book."""
        with psycopg.connect(posting.DATABASE_URL) as connection:
            connection.execute(
                "SELECT set_config('app.tenant_account_id', %s, false)",
                (str(self.case.tenant_id),),
            ...
            book_valid_from = anchor - timedelta(days=10)
            book_valid_to = anchor + timedelta(days=10)
            chart_valid_to = anchor + timedelta(days=12)
            assignment_valid_from = anchor - timedelta(days=1)
            assignment_valid_to = anchor + timedelta(days=5)
            extended_assignment_valid_to = anchor + timedelta(days=11)
            book_reference = f"urn:cwl:accounting_book:assignment_end_{uuid.uuid4().hex}"
            bank_account_reference = f"urn:cwl:bank_account:assignment_end_{uuid.uuid4().hex}"
            assignment_key = f"assignment-end-{uuid.uuid4().hex}"
            ...
            chart_account_id = connection.execute(
                ...
                INSERT INTO accounting_core.chart_account (
                    ...
                    valid_to,
                    account_class_code
                ) VALUES (%s, %s, '190902', 'Temporal assignment receivable', 'debit', %s, %s, 'asset')
                RETURNING chart_account_id
                """,
                (
                    self.case.tenant_id,
            ...
            self.assertLess(book_valid_to, extended_assignment_valid_to)
            self.assertLess(extended_assignment_valid_to, chart_valid_to)
            with self.assertRaises(psycopg.IntegrityError):
                connection.execute(
                    """
    ...
    def test_assignment_start_cannot_move_before_existing_book_start(self) -> None:
        """Moving an assignment start backward may not predate its Accounting Book."""
        with psycopg.connect(posting.DATABASE_URL) as connection:
            connection.execute(
                "SELECT set_config('app.tenant_account_id', %s, false)",
                (str(self.case.tenant_id),),
            ...
            anchor = connection.execute("SELECT clock_timestamp()").fetchone()[0]
            chart_valid_from = anchor - timedelta(days=12)
            book_valid_from = anchor - timedelta(days=10)
            book_valid_to = anchor + timedelta(days=10)
            assignment_valid_from = anchor - timedelta(days=1)
            assignment_valid_to = anchor + timedelta(days=5)
            moved_assignment_valid_from = anchor - timedelta(days=11)
            book_reference = f"urn:cwl:accounting_book:assignment_start_{uuid.uuid4().hex}"
            bank_account_reference = f"urn:cwl:bank_account:assignment_start_{uuid.uuid4().hex}"
            assignment_key = f"assignment-start-{uuid.uuid4().hex}"
            ...
            chart_account_id = connection.execute(
                ...
                INSERT INTO accounting_core.chart_account (
                    ...
                    valid_to,
                    account_class_code
                ) VALUES (%s, %s, '190903', 'Temporal assignment reserve', 'debit', %s, %s, 'asset')
                RETURNING chart_account_id
                """,
                (
                    self.case.tenant_id,
            ...
            self.assertLess(chart_valid_from, moved_assignment_valid_from)
            self.assertLess(moved_assignment_valid_from, book_valid_from)
            with self.assertRaises(psycopg.IntegrityError):
                connection.execute(
                    """
                    UPDATE accounting_core.bank_account_assignment
