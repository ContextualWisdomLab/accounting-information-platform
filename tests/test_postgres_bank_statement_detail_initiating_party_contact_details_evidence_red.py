"""PostgreSQL REDs for initiating-party Contact13 evidence in camt.053 details."""
...
from tests import (
    test_postgres_bank_statement_detail_ultimate_party_contact_details_evidence_red
    as ultimate_contact,
)
...
class BankStatementDetailInitiatingPartyContactDetailsEvidenceRedTests(unittest.TestCase):
    """Retain InitgPty/Pty/CtctDtls as purpose-bound non-reversible evidence."""
    ...
    def setUp(self) -> None:
        """Prepare one exception-safe initiating-party helper and Contact13 variants."""
        self.case = (
        ...
        self.variants = (
            ultimate_contact.BankStatementDetailUltimatePartyContactDetailsEvidenceRedTests
            ._contact_variants(self.base_contact)
        )

    def test_contact_values_presence_population_and_order_are_material(self) -> None:
        """Every Contact13 scalar, optional child, repeated Othr value and order is material."""
        baseline = parse_bank_statement_payload(
            self._with_contact(self.base_contact),
            CAMT053_MESSAGE_DEFINITION,
        )
    ...
    def test_contact_xml_layout_is_representation_only(self) -> None:
        """Whitespace inside CtctDtls changes raw bytes without changing semantics."""
        baseline_payload = self._with_contact(self.base_contact)
        needle = b"                  <CtctDtls>\n"
    ...
    def test_all_contact_changes_reach_complete_correction_boundary(self) -> None:
        """Accepted initiating-party Contact13 provenance cannot be silently replaced."""
        baseline_payload = self._with_contact(self.base_contact)
        for semantic, contact in self.variants.items():
            with self.subTest(semantic=semantic):
                ...
                initiating.accept_bank_statement_evidence(
                    self.case.helper._command(
                        baseline_payload,
                        f"initiating-contact-{semantic}-baseline",
                ...
                with self.assertRaisesRegex(
                    ...
                    initiating.accept_bank_statement_evidence(
                        self.case.helper._command(
                            self._with_contact(contact),
                            f"initiating-contact-{semantic}-changed",
                            bank_account_reference,
                        ),
                        posting.DATABASE_URL,
                        self.case.helper.case.policy.tenant_reference,
    ...
    def test_buyer_projection_keeps_all_contact_values_non_reversible(self) -> None:
        ...
        sensitive.update(
            {
                "name_prefix": "MADM",
                "name": "Initiating Confidential Treasury Contact",
                "phone_number": "+49-30-555-7811",
                "mobile_number": "+49-171-555-7811",
                "fax_number": "+49-30-555-7812",
                "url_address": "https://initiating-sensitive.example.invalid/contact",
                "email_address": "initiating-private@example.invalid",
                "email_purpose": "CONFIDENTIAL",
                "job_title": "Initiating Restricted Treasury Role",
                "responsibility": "Initiating Restricted Cash Operations",
                "department": "Initiating Restricted Treasury Unit",
                "other_contacts": [
                    {"channel_type": "TELE", "identification": "initiating-secret-one"},
                    {"channel_type": "SWIF", "identification": "initiating-secret-two"},
                ],
        ...
        absent = self.case._ingest_and_read_first_entry(
            self._with_contact(None),
            f"initiating-contact-none-{uuid.uuid4().hex}",
        )
        rich = self.case._ingest_and_read_first_entry(
            self._with_contact(sensitive),
            f"initiating-contact-rich-{uuid.uuid4().hex}",
        )
        ...
        buyer_scalars = set(self.case._scalar_leaves(rich_public))
        for source_value in source_values:
            with self.subTest(source_value=source_value):
                self.assertNotIn(source_value, buyer_scalars)

    def _with_contact(self, contact: dict[str, object] | None) -> bytes:
        """Return one InitgPty/Pty branch with optional Contact13 after Id."""
