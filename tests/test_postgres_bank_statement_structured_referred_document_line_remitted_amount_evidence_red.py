"""REDs for line-level remitted-amount evidence preservation in structured remittance."""
...
class BankStatementStructuredLineRemittedAmountEvidenceRedTests(unittest.TestCase):
    """Retain LineDtls remitted amount with the exact source line and currency."""
    ...
    def setUp(self) -> None:
        """Prepare two-line statements differing only in one remitted amount."""
        ...
        self.base_second_line_remitted_amount = Decimal("5000.10")
        self.changed_second_line_remitted_amount = Decimal("5001.10")
        ...
        self.changed_payload = self._replace_second_line_remitted_amount(
            self.base_payload,
            self.base_second_line_remitted_amount,
            self.changed_second_line_remitted_amount,
        )
        ...
        self.bank_account_reference = (
            f"urn:cwl:bank_account:structured-line-remitted:{uuid.uuid4().hex}"
        )
        accept_bank_account_record(
            {
    ...
    def test_line_remitted_amount_is_material_to_each_canonical_evidence_hash(self) -> None:
        """Bind one remitted-amount-only source change to the complete evidence chain."""
        self.assertEqual(
            self._replace_second_line_remitted_amount(
                self.changed_payload,
                self.changed_second_line_remitted_amount,
                self.base_second_line_remitted_amount,
            ),
    ...
    def test_changed_line_remitted_amount_requires_explicit_statement_correction(self) -> None:
        """Remitted-amount-only changes cannot silently replay one statement identity."""
        accepted = accept_bank_statement_evidence(
            self._command(self.base_payload, "line-remitted-base"),
            posting.DATABASE_URL,
            self.case.policy.tenant_reference,
        ...
        with self.assertRaisesRegex(AccountingValidationError, _CORRECTION_ERROR):
            accept_bank_statement_evidence(
                self._command(self.changed_payload, "line-remitted-changed"),
                posting.DATABASE_URL,
                self.case.policy.tenant_reference,
                artifact_store=self.store,
            )

    def test_buyer_read_keeps_remitted_amount_bound_to_its_source_line(self) -> None:
        """Buyer reads retain the exact remitted amount inside the exact source line."""
        accepted = accept_bank_statement_evidence(
            self._command(self.changed_payload, "line-remitted-lookup"),
            posting.DATABASE_URL,
            self.case.policy.tenant_reference,
            artifact_store=self.store,
        )
        ...
        self.assertEqual(
            ...
            self._structured_projection(self.changed_second_line_remitted_amount),
        )
    ...
    def _structured_projection(
        self,
        second_line_remitted_amount: Decimal,
    ) -> list[dict[str, object]]:
        """Return the canonical projection with one isolated line remitted amount."""
        ...
        if second_remitted.get("currency_code") != "KRW":
            raise AssertionError("second line remitted currency must remain KRW")
        second_remitted["amount"] = self._canonical_decimal(second_line_remitted_amount)
        return projection

    def _replace_second_line_remitted_amount(
        self,
        payload: bytes,
        current_amount: Decimal,
        replacement_amount: Decimal,
    ) -> bytes:
        """Replace only RmtdAmt nested in the second source line."""
        ...
        remitted_pattern = re.compile(r'(<RmtdAmt Ccy="KRW">)([^<]+)(</RmtdAmt>)')
        matches = list(remitted_pattern.finditer(line_segment))
        if len(matches) != 1:
            raise AssertionError("second line must contain exactly one KRW remitted amount")
        if Decimal(matches[0].group(2)) != current_amount:
            raise AssertionError("second line remitted amount did not match source value")

        replacement_text = format(replacement_amount, "f")
        changed_line = remitted_pattern.sub(
    ...
    def _command(self, payload: bytes, suffix: str) -> dict[str, object]:
        ...
        return {
            ...
            "ingestion_idempotency_key": (
                f"structured-line-remitted-{suffix}-{uuid.uuid4().hex}"
            ),
