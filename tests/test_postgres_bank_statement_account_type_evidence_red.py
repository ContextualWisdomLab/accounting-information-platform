...
class BankStatementAccountTypeEvidenceRedTests(unittest.TestCase):
    ...
    def test_statement_lookup_exposes_same_purpose_bound_account_type_hash(self) -> None:
        ...
        self.assertIsInstance(type_hash, str)
        self.assertRegex(type_hash or "", _HASH_PATTERN)
        ...
        self.assertEqual(document.get("account_type_evidence_hash"), type_hash)

    def _assert_type_evidence_difference(self, left: object, right: object) -> None:
        """Require one account-type change to propagate without changing Acct/Id identity."""
        ...
        self.assertIsInstance(left_hash, str)
        self.assertIsInstance(right_hash, str)
        self.assertRegex(left_hash or "", _HASH_PATTERN)
        self.assertRegex(right_hash or "", _HASH_PATTERN)
