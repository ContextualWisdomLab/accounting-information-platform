"""PostgreSQL REDs for camt.053 transaction-detail trading-party evidence."""
...
class BankStatementDetailTradingPartyEvidenceRedTests(unittest.TestCase):
    """Retain bank-reported trading-party evidence without promoting accounting authority."""
    ...
    def setUp(self) -> None:
        """Prepare value, choice-discriminator, and representation-control variants."""
        ...
        self.base = {"choice": "party", "name": "Trading Party Alpha"}
        self.variants = {
            "party-name": {"choice": "party", "name": "Trading Party Beta"},
            "choice": {"choice": "agent", "name": "Trading Party Alpha"},
        }
    ...
    def test_trading_party_value_and_choice_are_material_to_evidence_identity(self) -> None:
        """Party name and Party50Choice discriminator independently change retained identity."""
    ...
    def test_xml_layout_does_not_change_trading_party_semantics(self) -> None:
        """Whitespace inside TradgPty changes raw bytes, not its semantic evidence identity."""
    ...
    def test_changed_trading_party_requires_explicit_statement_correction(self) -> None:
        """Accepted trading-party evidence cannot be replaced by silent replay."""
    ...
    def test_buyer_read_preserves_trading_party_without_changing_amount_truth(self) -> None:
        """Tenant-scoped reads expose trading-party provenance while 25000 KRW stays authoritative."""
    ...
    def _expected_hash(value: dict[str, str]) -> str:
        """Digest the complete admitted TradingParty Party50Choice projection."""
        preimage = json.dumps(
            {
    ...
    @staticmethod
    def _trading_party_xml(value: dict[str, str]) -> str:
        """Serialize one schema-shaped TransactionParties12/TradgPty choice."""
        choice = value["choice"]
        name = value["name"]
        if choice == "party":
            body = (
            ...
            raise AssertionError(f"unsupported trading-party choice: {choice}")
        return "              <TradgPty>\n" + body + "              </TradgPty>\n"
    ...
    def _with_trading_party(
        ...
        """Insert TradgPty after the fixture debtor and before the RltdPties boundary."""
    ...
    def _command(self, payload: bytes, suffix: str) -> dict[str, object]:
        ...
        return {
            ...
            "ingestion_idempotency_key": (
                f"detail-trading-party-{suffix}-{uuid.uuid4().hex}"
            ),
