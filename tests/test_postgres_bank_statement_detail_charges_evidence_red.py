"""PostgreSQL REDs for bank-statement transaction charge evidence."""
...
class BankStatementDetailChargesEvidenceRedTests(unittest.TestCase):
    """Keep present TxDtls/Chrgs totals in normalized transaction evidence."""
    ...
    def setUp(self) -> None:
        """Create two statements differing only in the bank-reported total charges amount."""
    ...
    def test_present_total_charges_amount_changes_canonical_hashes(self) -> None:
        """Changing only Chrgs/TtlChrgsAndTaxAmt changes detail, entry, and statement identity."""
        self.assertNotEqual(
    ...
    def test_same_statement_identity_cannot_replay_changed_total_charges_amount(self) -> None:
        """Changed reported charges require correction, not silent replay."""
        accept_bank_statement_evidence(
    ...
    def test_entry_lookup_preserves_present_total_charges_amount(self) -> None:
        """Buyer-visible detail reads expose exact total charges amount and currency."""
        ...
        self.assertEqual(
            first_detail["total_charges_and_tax_amount"],
            self.first_total_charges_amount,
        )
        self.assertEqual(
            first_detail["total_charges_and_tax_currency_code"],
    ...
    def _with_total_charges_amount(
        ...
        """Insert one schema-shaped charges total between amount details and related parties."""
    ...
    def _command(self, payload: bytes, suffix: str) -> dict[str, object]:
        ...
        return {
            "tenant_reference": self.case.policy.tenant_reference,
            "bank_account_reference": self.bank_account_reference,
            "ingestion_idempotency_key": f"transaction-charges-{suffix}-{uuid.uuid4().hex}",
