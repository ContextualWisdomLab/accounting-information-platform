"""PostgreSQL REDs for debtor/creditor-agent clearing-system choice evidence."""
...
class BankStatementDebtorCreditorAgentClearingSystemChoiceEvidenceRedTests(
    unittest.TestCase
):
    """Retain ClrSysMmbId choice semantics for DbtrAgt and CdtrAgt."""
    ...
    def test_clearing_system_choice_is_material_for_debtor_and_creditor_agent(self) -> None:
        """Choice, value, member id, and optional choice absence change agent evidence."""
    ...
    def test_clearing_system_layout_is_representation_only_for_both_roles(self) -> None:
        """Whitespace inside ClrSysId changes raw bytes without changing semantics."""
        needle = b"                  <ClrSysId>\n"
        whitespace = b"                    \n"
    ...
    def test_clearing_system_change_requires_explicit_correction(self) -> None:
        """Accepted debtor/creditor clearing evidence cannot be replaced on replay."""
        role = "CdtrAgt"
        base_payload = self._payload(
            role,
            choice_kind="proprietary",
    ...
    def test_clearing_system_choice_stays_digest_only_on_buyer_projection(self) -> None:
        """Buyer reads add no reversible fields for debtor/creditor clearing identity."""
        for role, identity in self.case.AGENTS.items():
            with self.subTest(role=role):
                ...
                rich_reference = self.case._register_account(
                    rich_statement,
                    f"{role.lower()}-clearing-rich",
                )
                baseline_reference = self.case._register_account(
                    baseline_statement,
                    f"{role.lower()}-clearing-baseline",
                )
                rich_detail = self.case._ingest_and_read(
                    rich_payload,
                    rich_reference,
                    f"{role.lower()}-clearing-rich-{uuid.uuid4().hex}",
                )
                baseline_detail = self.case._ingest_and_read(
                    baseline_payload,
                    baseline_reference,
                    f"{role.lower()}-clearing-baseline-{uuid.uuid4().hex}",
                )
                digest_key = identity["digest_key"]
    ...
    def _payload(
        ...
        *,
        ...
        payload = self.case._with_agent(role, identity, deep=False)
        bicfi = identity["bicfi"]
        needle = f"                  <BICFI>{bicfi}</BICFI>\n".encode("utf-8")
        if payload.count(needle) != 1:
            raise AssertionError("target debtor/creditor role must contain exactly one BICFI")
