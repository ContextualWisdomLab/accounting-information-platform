...
class HomeTaxCommandBoundaryTests(unittest.TestCase):
    ...
    def test_whitespace_idempotency_key_fails_before_ledger_work(self) -> None:
        """Whitespace cannot become a durable HomeTax command identity."""
        payload = self._payload(include_key=True)
        payload["idempotency_key"] = "   \t  "
        with mock.patch(
            "accounting_information_platform.accept.PostgresPostingLedger",
    ...
    def test_non_string_idempotency_key_fails_before_ledger_work(self) -> None:
        """A non-string command identity cannot cross the durable boundary."""
        payload = self._payload(include_key=True)
        payload["idempotency_key"] = 123
        with mock.patch(
            "accounting_information_platform.accept.PostgresPostingLedger",
            _RecordingLedger,
        ):
