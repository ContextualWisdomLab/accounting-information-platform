"""PostgreSQL REDs for camt.053 transaction-detail trading-agent identity evidence."""
...
class BankStatementDetailTradingPartyAgentIdentificationEvidenceRedTests(unittest.TestCase):
    """Retain bank-reported trading-agent identity without promoting accounting authority."""
    ...
    def setUp(self) -> None:
        """Prepare BICFI, branch-identifier, and representation-control variants."""
    ...
    def test_agent_bicfi_and_branch_identifier_are_material_to_evidence_identity(self) -> None:
        """BICFI and branch Id independently change retained trading-party identity."""
    ...
    def test_xml_layout_does_not_change_trading_agent_identity(self) -> None:
        """Whitespace inside FinInstnId changes source bytes, not semantic identity."""
    ...
    def test_changed_trading_agent_identity_requires_explicit_correction(self) -> None:
        """Accepted trading-agent evidence cannot be replaced by silent replay."""
    ...
    def test_buyer_read_preserves_trading_agent_without_changing_amount_truth(self) -> None:
        """Tenant reads expose agent identity while the exact 25000 KRW fact is unchanged."""
    ...
    def _trading_party_xml(value: dict[str, object]) -> str:
        """Serialize Party50Choice/Agt with financial-institution and branch identity."""
        if value.get("choice") != "agent":
            raise AssertionError("trading-agent identity RED requires the Agt branch")
        ...
        bicfi = str(financial_institution["bicfi"])
        name = str(financial_institution["name"])
    ...
    def _with_trading_party(
        ...
        """Insert trading-agent identity after debtor evidence in RltdPties."""
    ...
    def _command(self, payload: bytes, suffix: str) -> dict[str, object]:
        ...
        return {
            ...
            "ingestion_idempotency_key": (
                f"detail-trading-agent-identification-{suffix}-{uuid.uuid4().hex}"
            ),
