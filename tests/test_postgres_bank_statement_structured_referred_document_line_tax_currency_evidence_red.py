"""REDs for line-level tax currency evidence preservation."""
...
class BankStatementStructuredLineTaxCurrencyEvidenceRedTests(unittest.TestCase):
    """Retain each LineDtls tax currency with its exact source line, type, and amount."""
    ...
    def setUp(self) -> None:
        """Prepare line-tax statements differing only in one tax currency."""
        ...
        self.local_tax_amount = Decimal("50.00")
        self.base_tax_currency_code = "KRW"
        self.changed_tax_currency_code = "USD"
        ...
        self.base_payload = tax_contract._with_line_taxes(
            line_payload,
            self.line_contract._decimal_text(self.local_tax_amount),
        )
        self.changed_payload = self._replace_second_line_local_tax_currency(
            self.base_payload,
            self.base_tax_currency_code,
            self.changed_tax_currency_code,
        )
        ...
        self.bank_account_reference = (
            f"urn:cwl:bank_account:structured-line-tax-currency:{uuid.uuid4().hex}"
        )
        accept_bank_account_record(
            {
    ...
    def test_line_tax_currency_is_material_to_each_canonical_evidence_hash(self) -> None:
        """Bind a tax-currency-only source change to the complete evidence chain."""
        self.assertEqual(
            self._replace_second_line_local_tax_currency(
                self.changed_payload,
                self.changed_tax_currency_code,
                self.base_tax_currency_code,
            ),
        ...
        base_projection = self._structured_projection(self.base_tax_currency_code)
        changed_projection = self._structured_projection(self.changed_tax_currency_code)
    ...
    def test_changed_line_tax_currency_requires_explicit_statement_correction(self) -> None:
        """Tax-currency-only source changes cannot silently replay one statement identity."""
        accepted = accept_bank_statement_evidence(
            self._command(self.base_payload, "line-tax-currency-base"),
            posting.DATABASE_URL,
            self.case.policy.tenant_reference,
        ...
        with self.assertRaisesRegex(AccountingValidationError, _CORRECTION_ERROR):
            accept_bank_statement_evidence(
                self._command(self.changed_payload, "line-tax-currency-changed"),
                posting.DATABASE_URL,
                self.case.policy.tenant_reference,
                artifact_store=self.store,
            )

    def test_buyer_read_keeps_tax_currency_bound_to_the_exact_line(self) -> None:
        """Buyer reads retain the changed tax currency with its line, type, and amount."""
        accepted = accept_bank_statement_evidence(
            self._command(self.changed_payload, "line-tax-currency-lookup"),
            posting.DATABASE_URL,
            self.case.policy.tenant_reference,
            artifact_store=self.store,
        )
        ...
        self.assertEqual(
            ...
            self._structured_projection(self.changed_tax_currency_code),
        )
    ...
    def _structured_projection(self, second_tax_currency_code: str) -> list[dict[str, object]]:
        """Return the exact line-tax projection with one source tax currency changed."""
        ...
        line_details[1]["tax_amounts"] = [
            ...
            {
                "type_code": "LOCL",
                "amount": self.line_contract._decimal_text(self.local_tax_amount),
                "currency_code": second_tax_currency_code,
            },
        ]
        return projection

    def _replace_second_line_local_tax_currency(
        self,
        payload: bytes,
        old_currency_code: str,
        new_currency_code: str,
    ) -> bytes:
        """Replace only the LOCL tax amount currency inside the identified second line."""
        ...
        amount_text = self.line_contract._decimal_text(self.local_tax_amount)
        old_tax_xml = (
            "<TaxAmt>\n"
            "                        <Tp><Cd>LOCL</Cd></Tp>\n"
        ...
        if segment.count(old_tax_xml) != 1:
            raise AssertionError("target second-line LOCL tax amount must occur exactly once")
        if old_tax_xml != new_tax_xml and segment.count(new_tax_xml) != 0:
            raise AssertionError("replacement second-line LOCL tax currency must not pre-exist")

        replaced_segment = segment.replace(old_tax_xml, new_tax_xml, 1)
    ...
    def _command(self, payload: bytes, suffix: str) -> dict[str, object]:
        ...
        return {
            ...
            "ingestion_idempotency_key": (
                f"structured-line-tax-currency-{suffix}-{uuid.uuid4().hex}"
            ),
