"""REDs for line-level credit-note currency evidence preservation."""
...
class BankStatementStructuredLineCreditNoteCurrencyEvidenceRedTests(unittest.TestCase):
    """Retain each LineDtls credit-note currency with its exact line and amount."""
    ...
    def setUp(self) -> None:
        """Prepare two-line statements differing only in one credit-note currency."""
        ...
        self.first_line_credit_note_amount = Decimal("1200")
        self.second_line_credit_note_amount = Decimal("800")
        self.base_credit_note_currency_code = "KRW"
        self.changed_credit_note_currency_code = "USD"
        ...
        self.base_payload = self._with_line_credit_notes(line_payload)
        self.changed_payload = self._replace_second_line_credit_note_currency(
            self.base_payload,
            self.base_credit_note_currency_code,
            self.changed_credit_note_currency_code,
        )
        ...
        self.bank_account_reference = (
            f"urn:cwl:bank_account:structured-line-credit-note-currency:"
            f"{uuid.uuid4().hex}"
        )
        accept_bank_account_record(
            {
    ...
    def test_line_credit_note_currency_is_material_to_each_canonical_evidence_hash(
        self,
    ) -> None:
        """Bind one credit-note-currency-only source change to the evidence chain."""
        self.assertEqual(
            self._replace_second_line_credit_note_currency(
                self.changed_payload,
                self.changed_credit_note_currency_code,
                self.base_credit_note_currency_code,
            ),
    ...
    def test_changed_line_credit_note_currency_requires_explicit_statement_correction(
        self,
    ) -> None:
        """Credit-note-currency-only changes cannot silently replay one statement."""
        accepted = accept_bank_statement_evidence(
            self._command(self.base_payload, "line-credit-note-currency-base"),
            posting.DATABASE_URL,
            self.case.policy.tenant_reference,
        ...
        with self.assertRaisesRegex(AccountingValidationError, _CORRECTION_ERROR):
            accept_bank_statement_evidence(
                self._command(
                    self.changed_payload,
                    "line-credit-note-currency-changed",
                ),
                posting.DATABASE_URL,
                self.case.policy.tenant_reference,
                artifact_store=self.store,
    ...
    def test_buyer_read_keeps_credit_note_currency_bound_to_exact_line_and_amount(
        self,
    ) -> None:
        """Buyer reads retain credit-note currency with its exact source tuple."""
        accepted = accept_bank_statement_evidence(
            self._command(
                self.changed_payload,
                "line-credit-note-currency-lookup",
            ),
            posting.DATABASE_URL,
            self.case.policy.tenant_reference,
            artifact_store=self.store,
        ...
        self.assertEqual(
            ...
            self._structured_projection(self.changed_credit_note_currency_code),
        )
    ...
    def _structured_projection(
        self,
        second_line_credit_note_currency_code: str,
    ) -> list[dict[str, object]]:
        """Return the canonical projection with one isolated credit-note currency."""
        ...
        line_details[0]["credit_note_amount"] = {
            "amount": self.line_contract._decimal_text(
                self.first_line_credit_note_amount
            ),
            "currency_code": self.base_credit_note_currency_code,
        }
        line_details[1]["credit_note_amount"] = {
            "amount": self.line_contract._decimal_text(
                self.second_line_credit_note_amount
            ),
            "currency_code": second_line_credit_note_currency_code,
        }
        return projection

    def _with_line_credit_notes(self, line_payload: bytes) -> bytes:
        """Insert one credit note into each source line before remitted amount."""
        ...
        first_credit_note = (
            f'                      <CdtNoteAmt Ccy="KRW">'
            f"{format(self.first_line_credit_note_amount, 'f')}</CdtNoteAmt>\n"
            f"{first_marker}"
        )
        second_credit_note = (
            f'                      <CdtNoteAmt Ccy="KRW">'
            f"{format(self.second_line_credit_note_amount, 'f')}</CdtNoteAmt>\n"
            f"{second_marker}"
        )
        text = text.replace(first_marker, first_credit_note, 1)
        text = text.replace(second_marker, second_credit_note, 1)
        return text.encode("utf-8")

    def _replace_second_line_credit_note_currency(
        self,
        payload: bytes,
        old_currency_code: str,
        new_currency_code: str,
    ) -> bytes:
        """Replace only credit-note currency nested in the second source line."""
        ...
        amount_text = format(self.second_line_credit_note_amount, "f")
        old_xml = f'<CdtNoteAmt Ccy="{old_currency_code}">{amount_text}</CdtNoteAmt>'
        new_xml = f'<CdtNoteAmt Ccy="{new_currency_code}">{amount_text}</CdtNoteAmt>'
        if line_segment.count(old_xml) != 1:
            raise AssertionError("target second-line credit note must occur exactly once")
        if old_xml != new_xml and line_segment.count(new_xml) != 0:
            raise AssertionError("replacement credit-note currency must not pre-exist")
        if line_segment.count("<CdtNoteAmt ") != 1:
            raise AssertionError("second line must contain exactly one credit-note amount")

        changed_line = line_segment.replace(old_xml, new_xml, 1)
    ...
    def _command(self, payload: bytes, suffix: str) -> dict[str, object]:
        ...
        return {
            ...
            "ingestion_idempotency_key": (
                f"structured-line-credit-note-currency-{suffix}-{uuid.uuid4().hex}"
            ),
