"""REDs for member-local Related Date inside repeated referred-document line Identifications."""
...
from tests import (
    test_postgres_bank_statement_structured_referred_document_repeated_line_identification_number_evidence_red
    as number_contract,
)

_PARENT_TEST = (
    number_contract.BankStatementStructuredRepeatedLineIdentificationNumberEvidenceRedTests
)
_NORMALIZATION_PARENT_TEST = number_contract._ISSUER_PARENT_TEST
_CORRECTION_ERROR = number_contract._CORRECTION_ERROR
_STRUCTURED_EVIDENCE_KEY = "structured_referred_document_evidence"


class BankStatementStructuredRepeatedLineIdentificationRelatedDateEvidenceRedTests(
    unittest.TestCase
):
    """Retain Related Date on the exact repeated line-identification member that supplied it."""

    @classmethod
    def setUpClass(cls) -> None:
        """Reuse the real-PostgreSQL repeated-Id Number fixture."""
        _PARENT_TEST.setUpClass()

    def setUp(self) -> None:
        """Mutate only first-member Related Date while holding Type/Issuer/Number stable."""
        self.parent = _PARENT_TEST(
            "test_repeated_identification_number_is_material_to_each_hash"
        )
        self.parent.setUp()
        self.addCleanup(self.parent.doCleanups)
        self.line_contract = self.parent.line_contract

        self.base_number = self.parent.base_number
        self.base_related_date = self._first_repeated_related_date(
            self.parent.base_projection
        )
        self.changed_related_date = "2026-09-03"
        if self.changed_related_date == self.base_related_date:
            raise AssertionError("related-date RED requires a distinct changed date")
        self.base_payload = self.parent.base_payload
        self.changed_payload = self._replace_first_repeated_related_date(
            self.base_payload,
        ...
        self.changed_projection = self._projection_with_first_repeated_related_date(
            self.changed_related_date
        )

    def test_repeated_identification_related_date_is_material_to_each_hash(
        self,
    ) -> None:
        """Bind a member-local Related-Date-only source change to every evidence hash."""
        self.assertEqual(
            self._replace_first_repeated_related_date(
                self.changed_payload,
                self.changed_related_date,
                self.base_related_date,
            ),
    ...
    def test_rejected_repeated_identification_related_date_change_preserves_all_evidence(
        self,
    ) -> None:
        """Reject a Related-Date-only replay without relational or raw-artifact residue."""
        restricted_owner = self.parent.parent.parent.parent.parent
        runtime_url = restricted_owner._restricted_bank_statement_runtime_url()
        accepted = accept_bank_statement_evidence(
            self.line_contract._command(
                self.base_payload,
                "repeated-line-identification-related-date-base",
            ),
            runtime_url,
        ...
        with self.assertRaisesRegex(AccountingValidationError, _CORRECTION_ERROR):
            accept_bank_statement_evidence(
                self.line_contract._command(
                    self.changed_payload,
                    "repeated-line-identification-related-date-changed",
                ),
                runtime_url,
                self.line_contract.case.policy.tenant_reference,
                artifact_store=self.line_contract.store,
    ...
    def test_buyer_read_keeps_related_date_bound_to_exact_repeated_member(
        self,
    ) -> None:
        """Expose changed Related Date only on the first repeated identification."""
        accepted = accept_bank_statement_evidence(
            self.line_contract._command(
                self.changed_payload,
                "repeated-line-identification-related-date-lookup",
            ),
        ...
        self.assertEqual(
            ...
            self.parent.parent.parent.sknb_issuer,
        )
        self.assertEqual(
            second_line["line_identifications"],
            [
                {
                    "type_proprietary": self.parent.parent.base_proprietary_type,
                    "issuer": self.parent.parent.parent.prnb_issuer,
                    "number": self.base_number,
                    "related_date": self.changed_related_date,
                },
                {
                    "type_code": "SKNB",
                    "issuer": self.parent.parent.parent.sknb_issuer,
    ...
    def _projection_with_first_repeated_related_date(
        self,
        related_date: str,
    ) -> list[dict[str, object]]:
        """Change only Related Date on the proprietary first repeated identification."""
        projection = deepcopy(self.parent.base_projection)
        first_member, second_member = self.parent._repeated_members(projection)
        if (
            first_member.get("type_proprietary")
            != self.parent.parent.base_proprietary_type
        ):
            raise AssertionError("first repeated Id must retain proprietary Type")
        if first_member.get("issuer") != self.parent.parent.parent.prnb_issuer:
            raise AssertionError("first repeated Id must retain PARTS-SCHEME issuer")
        if first_member.get("number") != self.base_number:
            raise AssertionError("first repeated Id must retain canonical Number")
        if first_member.get("related_date") != self.base_related_date:
            raise AssertionError("first repeated Id must begin with canonical Related Date")
        first_member["related_date"] = related_date
        if second_member.get("type_code") != "SKNB":
            raise AssertionError("second repeated Id must remain canonical SKNB")
        if second_member.get("related_date") != self.line_contract.line_related_date:
            raise AssertionError("second repeated Id Related Date must remain canonical")
        return projection

    def _replace_first_repeated_related_date(
        self,
        payload: bytes,
        current_date: str,
        replacement_date: str,
    ) -> bytes:
        """Replace Related Date only on the proprietary first repeated Id member."""
        text = payload.decode("utf-8")
        line_segment, line_start, line_end = (
            self.parent.parent.parent.parent.parent._second_line_segment(text)
        )
        target_type = self.parent.parent.base_proprietary_type
        target_issuer = self.parent.parent.parent.prnb_issuer
        target_number = self.base_number
        starts: list[int] = []
        cursor = 0
        while True:
        ...
        if len(starts) != 2 or line_segment.count("</Id>") != 2:
            raise AssertionError(
                "repeated-Id Related-Date RED requires exactly two Id blocks"
            )
        ...
        current_markup = f"<RltdDt>{current_date}</RltdDt>"
        replacement_markup = f"<RltdDt>{replacement_date}</RltdDt>"
        if block.count(current_markup) != 1:
            raise AssertionError(
                "target repeated Id Related Date must occur exactly once"
            )
        if current_markup != replacement_markup and replacement_markup in block:
            raise AssertionError(
                "replacement repeated Id Related Date must not pre-exist"
            )
    ...
    def _first_repeated_related_date(
        ...
        first_member, _ = self.parent._repeated_members(projection)
