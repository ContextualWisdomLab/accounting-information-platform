...
from tests import (
    ...
    as optional_due_payable_contract,
)

_PARENT_TEST = (
    optional_due_payable_contract.
    BankStatementStructuredLineOptionalDuePayableAmountAbsenceEvidenceRedTests
)
_NORMALIZATION_PARENT_TEST = optional_due_payable_contract._NORMALIZATION_PARENT_TEST
_CORRECTION_ERROR = optional_due_payable_contract._CORRECTION_ERROR
_STRUCTURED_EVIDENCE_KEY = "structured_referred_document_evidence"
_TARGET_KEY = "remitted_amount"
_OTHER_AMOUNT_KEYS = tuple(
    key
    for key in optional_due_payable_contract.optional_amount_contract._AMOUNT_EVIDENCE_KEYS
    if key != _TARGET_KEY
)
...
class BankStatementStructuredLineOptionalRemittedAmountAbsenceEvidenceRedTests(
    ...
    def setUp(self) -> None:
        """Remove only line-two RmtdAmt while preserving remaining Amount evidence."""
        ...
        # Compose on #117's changed source: DuePyblAmt remains absent while the
        # direct Amount group and source-populated discount evidence stay live.
        ...
        self.changed_projection = self._projection_without_second_line_remitted_amount()

    def test_optional_remitted_amount_absence_is_material_to_each_evidence_hash(
        self,
    ) -> None:
    ...
    def test_rejected_remitted_amount_absence_change_leaves_no_evidence_residue(
        self,
    ) -> None:
        """Reject RmtdAmt omission without changing relational or raw evidence."""
        restricted_owner = self.parent.parent.parent._ancestor_with(
    ...
    def test_buyer_read_keeps_remitted_amount_absent_on_exact_source_line(self) -> None:
        """Expose no line-two Remitted Amount while retaining discount evidence."""
        accepted = accept_bank_statement_evidence(
            self.line_contract._command(
                self.changed_payload,
        ...
        # Parent omission contracts remain live.
    ...
    def _projection_without_second_line_remitted_amount(
        self,
    ) -> list[dict[str, object]]:
        """Remove only Remitted Amount from line two's canonical projection."""
        projection = deepcopy(self.base_projection)
        first_line, second_line = self.parent.parent._line_details(projection)
        if _TARGET_KEY not in second_line:
            raise AssertionError("second source line must begin with Remitted Amount")
        ...
        for key in self.second_line_absent_optional_amount_keys:
            if key in second_line:
                raise AssertionError(
                    "Remitted Amount omission must not manufacture absent optional evidence"
                )
        for key, expected in self.first_line_amount_evidence.items():
    ...
    def _extract_second_line_remitted_line(self, payload: bytes) -> str:
        ...
        if any(candidate.strip() for candidate in lines[index + 1 : end]):
    ...
    def _remove_second_line_remitted_amount(self, payload: bytes) -> bytes:
        ...
        text = payload.decode("utf-8")
        segment, segment_start, segment_end = self.parent.parent._second_line_segment(text)
    ...
    def _restore_second_line_remitted_amount(self, payload: bytes) -> bytes:
        """Restore the exact removed RmtdAmt line before direct Amount closes."""
        text = payload.decode("utf-8")
        segment, segment_start, segment_end = self.parent.parent._second_line_segment(text)
        lines = segment.splitlines(keepends=True)
