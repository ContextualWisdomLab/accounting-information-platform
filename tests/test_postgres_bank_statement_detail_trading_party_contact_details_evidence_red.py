"""PostgreSQL REDs for trading-party contact-detail evidence."""
...
class BankStatementDetailTradingPartyContactDetailsEvidenceRedTests(unittest.TestCase):
    """Retain PartyIdentification272/CtctDtls as evidence, never accounting authority."""
    ...
    def setUp(self) -> None:
        """Prepare Contact13 value, optionality, and representation-control variants."""
    ...
    def test_contact_values_and_presence_are_material_to_evidence_identity(self) -> None:
        """Contact13 fields and container presence independently change retained identity."""
    ...
    def test_xml_layout_does_not_change_contact_detail_semantics(self) -> None:
        """Whitespace inside CtctDtls changes raw bytes, not semantic evidence identity."""
    ...
    def test_changed_contact_details_require_explicit_statement_correction(self) -> None:
        """Accepted contact evidence cannot be replaced by silent statement replay."""
    ...
    def test_buyer_read_preserves_contact_details_without_changing_amount_truth(self) -> None:
        """Tenant reads expose Contact13 provenance while 25000 KRW remains authoritative."""
        expected_hash = self._expected_hash(self.base)
        accepted = accept_bank_statement_evidence(
    ...
    def _assert_accounting_truth(entry: object, detail: object) -> None:
        """Pin bank-reported contact provenance away from authoritative accounting amount."""
    ...
    def _expected_hash(value: dict[str, object]) -> str:
        """Digest the admitted PartyIdentification272 contact-detail projection."""
        preimage = json.dumps(
            {
    ...
    def _trading_party_xml(value: dict[str, object]) -> str:
        """Serialize one PartyIdentification272 with optional Contact13 details."""
        if value.get("choice") != "party":
            raise AssertionError("contact-detail RED requires the Pty branch")
        name = str(value["name"])
    ...
    def _with_trading_party(
        ...
        """Insert contact-bearing trading-party evidence after fixture debtor evidence."""
    ...
    def _command(self, payload: bytes, suffix: str) -> dict[str, object]:
        ...
        return {
            ...
            "ingestion_idempotency_key": (
                f"detail-trading-party-contact-{suffix}-{uuid.uuid4().hex}"
            ),
