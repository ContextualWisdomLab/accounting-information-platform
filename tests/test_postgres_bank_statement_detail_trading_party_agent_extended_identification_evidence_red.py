"""PostgreSQL REDs for complete camt.053 trading-agent identification evidence."""
...
class BankStatementDetailTradingPartyAgentExtendedIdentificationEvidenceRedTests(
    unittest.TestCase
):
    """Retain clearing-member and LEI evidence without granting accounting authority."""
    ...
    def setUp(self) -> None:
        """Prepare complete FinancialInstitutionIdentification23 and BranchData5 variants."""
        ...
        self.variants = {
            "clearing-member-id": copy.deepcopy(self.base),
            "financial-institution-lei": copy.deepcopy(self.base),
            "branch-lei": copy.deepcopy(self.base),
            "branch-name": copy.deepcopy(self.base),
        }
        self.variants["clearing-member-id"]["financial_institution"][
            "clearing_system_member"
        ]["member_id"] = "DE-CLEAR-002"
        self.variants["financial-institution-lei"]["financial_institution"][
            "lei"
        ] = "529900T8BM49AURSDO55"
        self.variants["branch-lei"]["branch"]["lei"] = "529900R2B8Z9HUX3B787"
        self.variants["branch-name"]["branch"]["name"] = "Frankfurt Settlement Branch"
    ...
    def test_extended_agent_identifiers_are_material_to_evidence_identity(self) -> None:
        """Clearing member, institution LEI, branch LEI, and branch name stay material."""
    ...
    def test_xml_layout_does_not_change_extended_agent_identity(self) -> None:
        """Whitespace inside ClrSysMmbId changes source bytes, not semantic identity."""
    ...
    def test_changed_extended_agent_identity_requires_explicit_correction(self) -> None:
        """Accepted trading-agent identifiers cannot be replaced by silent replay."""
    ...
    def test_buyer_read_preserves_extended_agent_identity_without_changing_amount(self) -> None:
        """Tenant reads expose complete agent identity while exact 25000 KRW stays fixed."""
    ...
    def _assert_exact_accounting_amount(entry: object, detail: object) -> None:
        """Pin the accounting fact independently from bank-reported agent evidence."""
        if getattr(entry, "entry_amount", None) != Decimal("25000.00"):
            raise AssertionError("trading-agent evidence must retain exact 25000.00 entry amount")
        if getattr(entry, "entry_currency_code", None) != "KRW":
            raise AssertionError("trading-agent evidence must retain KRW entry currency")
        if getattr(detail, "detail_amount", None) != Decimal("25000.00"):
            raise AssertionError("trading-agent evidence must retain exact 25000.00 detail amount")
        if getattr(detail, "detail_currency_code", None) != "KRW":
            raise AssertionError("trading-agent evidence must retain KRW detail currency")
    ...
    def _trading_party_xml(value: dict[str, object]) -> str:
        """Serialize V14 agent identity through FinancialInstitutionIdentification23."""
        if value.get("choice") != "agent":
            raise AssertionError("extended trading-agent RED requires the Agt branch")
        ...
        clearing_system_member = financial_institution.get("clearing_system_member")
        if not isinstance(clearing_system_member, dict):
            raise AssertionError("financial institution must provide clearing member identity")
        return (
            "              <TradgPty>\n"
            "                <Agt>\n"
            "                  <FinInstnId>\n"
            ...
            "                    <ClrSysMmbId>\n"
            f"                      <MmbId>{clearing_system_member['member_id']}</MmbId>\n"
            "                    </ClrSysMmbId>\n"
    ...
    def _with_trading_party(
        ...
        """Insert complete trading-agent identity after debtor evidence in RltdPties."""
    ...
    def _command(self, payload: bytes, suffix: str) -> dict[str, object]:
        ...
        return {
            ...
            "ingestion_idempotency_key": (
                f"detail-trading-agent-extended-{suffix}-{uuid.uuid4().hex}"
            ),
