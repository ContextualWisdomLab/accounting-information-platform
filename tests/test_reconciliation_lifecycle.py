...
class _Connection:
    ...
    def execute(
        ...
        if "FROM accounting_core.reconciliation_exception_resolution_command" in normalized:
    ...
    def commit(self) -> None:
        """Record an explicit transaction boundary used by the session-lease protocol."""
        self.commit_count += 1

    def rollback(self) -> None:
        """Record an explicit rollback used by the session-lease protocol."""
...
class ReconciliationLifecycleTests(unittest.TestCase):
    ...
    def test_reused_transition_key_rejects_each_changed_identity_field(self) -> None:
        ...
        cases = (
            ((UUID("33333333-3333-3333-3333-333333333333"), "urn:cwl:principal:controller", "month_end_reconciliation", _EFFECTIVE_AT, _SOURCE_PAYLOAD_HASH), "run"),
            ((_RUN_ID, "urn:cwl:principal:other", "month_end_reconciliation", _EFFECTIVE_AT, _SOURCE_PAYLOAD_HASH), "actor"),
            ((_RUN_ID, "urn:cwl:principal:controller", "other_purpose", _EFFECTIVE_AT, _SOURCE_PAYLOAD_HASH), "purpose"),
            ((_RUN_ID, "urn:cwl:principal:controller", "month_end_reconciliation", datetime(2026, 9, 1, 12, 2, tzinfo=timezone.utc), _SOURCE_PAYLOAD_HASH), "effective"),
            ((_RUN_ID, "urn:cwl:principal:controller", "month_end_reconciliation", _EFFECTIVE_AT, "sha256:" + "9" * 64), "source"),
        )
...
class ReconciliationLifecycleMigrationContractTests(unittest.TestCase):
    ...
    def test_exception_resolution_migration_requires_named_maker_checker_command(self) -> None:
        """Migration 0022 replaces mutable exception status with durable command authority."""
        from pathlib import Path

        root = Path(__file__).resolve().parents[1]
        migration = (
            root / "database/migrations/0022_reconciliation_exception_resolution_command.sql"
        ).read_text(encoding="utf-8")
        self.assertIn(
    ...
    def test_lifecycle_source_payload_identity_is_forward_only_and_database_bound(self) -> None:
        """Migration 0028 persists strict source identity without inventing legacy hashes."""
        from pathlib import Path

        root = Path(__file__).resolve().parents[1]
        migration = (
            root / "database/migrations/0028_reconciliation_lifecycle_source_payload_identity.sql"
        ).read_text(encoding="utf-8")
