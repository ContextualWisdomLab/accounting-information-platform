"""REDs for referred-document line-identification related-date evidence preservation."""
...
class BankStatementStructuredLineRelatedDateEvidenceRedTests(unittest.TestCase):
    """Retain LineDtls/Id/RltdDt with the exact source line identification."""

    @classmethod
    def setUpClass(cls) -> None:
        """Reuse the canonical real-PostgreSQL integration fixture."""
        line_code_contract.BankStatementStructuredLineIdentificationCodeEvidenceRedTests.setUpClass()

    def setUp(self) -> None:
        """Prepare statements differing only in the second line related date."""
        self.code_contract = (
            line_code_contract.BankStatementStructuredLineIdentificationCodeEvidenceRedTests(
                "test_line_identification_code_is_material_to_each_canonical_evidence_hash"
            )
        )
        self.code_contract.setUp()
        self.addCleanup(self.code_contract.doCleanups)
        self.line_contract = self.code_contract.line_contract

        self.base_related_date = self.line_contract.line_related_date
        self.changed_related_date = "2026-09-02"
        if self.base_related_date == self.changed_related_date:
            raise AssertionError("changed line related date must differ from the canonical date")
        self.base_payload = self.code_contract.base_payload
        self.changed_payload = self._replace_second_line_related_date(
            self.base_payload,
    ...
    def test_line_related_date_is_material_to_each_canonical_evidence_hash(self) -> None:
        """Bind a second-line related-date-only change to every canonical evidence hash."""
        self.assertEqual(
            self._replace_second_line_related_date(
                self.changed_payload,
                self.changed_related_date,
                self.base_related_date,
            ),
        ...
        base_projection = self._structured_projection(self.base_related_date)
        changed_projection = self._structured_projection(self.changed_related_date)
    ...
    def test_changed_line_related_date_fails_closed_for_same_statement_identity(self) -> None:
        """A line-related-date-only change fails closed without correction authority."""
        accepted = accept_bank_statement_evidence(
            self.line_contract._command(self.base_payload, "line-related-date-base"),
    ...
    def test_buyer_read_keeps_related_date_bound_to_exact_line(self) -> None:
        """Buyer reads retain the changed date on the exact second source line."""
        accepted = accept_bank_statement_evidence(
            self.line_contract._command(
                self.changed_payload,
                "line-related-date-lookup",
            ),
            posting.DATABASE_URL,
            self.line_contract.case.policy.tenant_reference,
            artifact_store=self.line_contract.store,
        ...
        self.assertEqual(
            ...
            self._structured_projection(self.changed_related_date),
        )
    ...
    def _structured_projection(self, second_line_related_date: str) -> list[dict[str, object]]:
        """Return the ordered projection with only the second line related date changed."""
        projection = deepcopy(
            self.code_contract._structured_projection(self.code_contract.base_line_type_code)
        )
        ...
        if first_line.get("related_date") != self.base_related_date:
            raise AssertionError("first source line must retain the canonical related date")
        if second_line.get("related_date") != self.base_related_date:
            raise AssertionError("parent second source line must retain the canonical related date")
        second_line["related_date"] = second_line_related_date
        return projection

    def _replace_second_line_related_date(
        self,
        payload: bytes,
        old_related_date: str,
        new_related_date: str,
    ) -> bytes:
        """Replace only Id/RltdDt inside the exact second source line."""
        ...
        old_markup = f"<RltdDt>{old_related_date}</RltdDt>"
        new_markup = f"<RltdDt>{new_related_date}</RltdDt>"
        if line_segment.count(old_markup) != 1:
            raise AssertionError("second line related-date marker must occur exactly once")
        if old_markup != new_markup and line_segment.count(new_markup) != 0:
            raise AssertionError("replacement line related date must not pre-exist")
