"""REDs for line-level due-payable currency evidence preservation."""
...
class BankStatementStructuredLineDuePayableCurrencyEvidenceRedTests(unittest.TestCase):
    """Retain each LineDtls due-payable currency with its exact source line and amount."""
    ...
    def setUp(self) -> None:
        """Prepare two-line statements differing only in one due-payable currency."""
        ...
        self.second_line_due_payable_amount = Decimal("5100.10")
        self.base_due_payable_currency_code = "KRW"
        self.changed_due_payable_currency_code = "USD"
        ...
        self.changed_payload = self._replace_second_line_due_payable_currency(
            self.base_payload,
            self.base_due_payable_currency_code,
            self.changed_due_payable_currency_code,
        )
        ...
        self.bank_account_reference = (
            f"urn:cwl:bank_account:structured-line-due-payable-currency:{uuid.uuid4().hex}"
        )
        accept_bank_account_record(
            {
    ...
    def test_line_due_payable_currency_is_material_to_each_canonical_evidence_hash(
        self,
    ) -> None:
        """Bind one due-payable-currency-only source change to the evidence chain."""
        self.assertEqual(
            self._replace_second_line_due_payable_currency(
                self.changed_payload,
                self.changed_due_payable_currency_code,
                self.base_due_payable_currency_code,
            ),
    ...
    def test_changed_line_due_payable_currency_requires_explicit_statement_correction(
        self,
    ) -> None:
        """Due-payable-currency-only changes cannot silently replay one statement."""
        accepted = accept_bank_statement_evidence(
            self._command(self.base_payload, "line-due-payable-currency-base"),
            posting.DATABASE_URL,
            self.case.policy.tenant_reference,
        ...
        with self.assertRaisesRegex(AccountingValidationError, _CORRECTION_ERROR):
            accept_bank_statement_evidence(
                self._command(
                    self.changed_payload,
                    "line-due-payable-currency-changed",
                ),
                posting.DATABASE_URL,
                self.case.policy.tenant_reference,
                artifact_store=self.store,
    ...
    def test_buyer_read_keeps_due_payable_currency_bound_to_its_source_line(
        self,
    ) -> None:
        """Buyer reads retain due-payable currency with the exact source line and amount."""
        accepted = accept_bank_statement_evidence(
            self._command(
                self.changed_payload,
                "line-due-payable-currency-lookup",
            ),
            posting.DATABASE_URL,
            self.case.policy.tenant_reference,
            artifact_store=self.store,
        ...
        self.assertEqual(
            ...
            self._structured_projection(self.changed_due_payable_currency_code),
        )
    ...
    def _structured_projection(
        self,
        second_line_due_payable_currency_code: str,
    ) -> list[dict[str, object]]:
        """Return the canonical projection with one isolated due-payable currency."""
        ...
        if Decimal(str(second_due.get("amount"))) != self.second_line_due_payable_amount:
            raise AssertionError("second line due-payable amount must remain exact")
        second_due["currency_code"] = second_line_due_payable_currency_code
        return projection

    def _replace_second_line_due_payable_currency(
        self,
        payload: bytes,
        old_currency_code: str,
        new_currency_code: str,
    ) -> bytes:
        """Replace only DuePyblAmt currency nested in the identified second line."""
        ...
        amount_text = format(self.second_line_due_payable_amount, "f")
        old_xml = (
            f'<DuePyblAmt Ccy="{old_currency_code}">{amount_text}</DuePyblAmt>'
        )
        new_xml = (
            f'<DuePyblAmt Ccy="{new_currency_code}">{amount_text}</DuePyblAmt>'
        )
        if line_segment.count(old_xml) != 1:
            raise AssertionError(
                "target second-line due-payable amount must occur exactly once"
            )
        if old_xml != new_xml and line_segment.count(new_xml) != 0:
            raise AssertionError(
                "replacement second-line due-payable currency must not pre-exist"
            )
    ...
    def _command(self, payload: bytes, suffix: str) -> dict[str, object]:
        ...
        return {
            ...
            "ingestion_idempotency_key": (
                f"structured-line-due-payable-currency-{suffix}-{uuid.uuid4().hex}"
            ),
