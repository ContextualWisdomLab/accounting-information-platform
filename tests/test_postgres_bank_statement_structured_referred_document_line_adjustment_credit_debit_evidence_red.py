"""REDs for line-level adjustment credit/debit evidence preservation."""
...
class BankStatementStructuredLineAdjustmentCreditDebitEvidenceRedTests(unittest.TestCase):
    """Retain each line adjustment credit/debit indicator with its exact source tuple."""
    ...
    def setUp(self) -> None:
        """Prepare two-line statements differing only in one adjustment indicator."""
        ...
        self.adjustment_currency_code = "KRW"
        self.base_credit_debit_code = "DBIT"
        self.changed_credit_debit_code = "CRDT"
        ...
        self.changed_payload = self._replace_second_line_later_adjustment_indicator(
            self.base_payload,
            self.base_credit_debit_code,
            self.changed_credit_debit_code,
        )
        ...
        self.bank_account_reference = (
            f"urn:cwl:bank_account:structured-line-adjustment-credit-debit:"
            f"{uuid.uuid4().hex}"
        )
        accept_bank_account_record(
            {
    ...
    def test_line_adjustment_indicator_is_material_to_each_canonical_evidence_hash(
        self,
    ) -> None:
        """Bind one adjustment-indicator-only source change to the evidence chain."""
        self.assertEqual(
            self._replace_second_line_later_adjustment_indicator(
                self.changed_payload,
                self.changed_credit_debit_code,
                self.base_credit_debit_code,
            ),
        ...
        base_projection = self._structured_projection(self.base_credit_debit_code)
        changed_projection = self._structured_projection(self.changed_credit_debit_code)
    ...
    def test_changed_line_adjustment_indicator_requires_explicit_statement_correction(
        self,
    ) -> None:
        """Adjustment-indicator-only changes cannot silently replay one statement."""
        accepted = accept_bank_statement_evidence(
            self._command(self.base_payload, "line-adjustment-indicator-base"),
            posting.DATABASE_URL,
            self.case.policy.tenant_reference,
        ...
        with self.assertRaisesRegex(AccountingValidationError, _CORRECTION_ERROR):
            accept_bank_statement_evidence(
                self._command(
                    self.changed_payload,
                    "line-adjustment-indicator-changed",
                ),
                posting.DATABASE_URL,
                self.case.policy.tenant_reference,
                artifact_store=self.store,
    ...
    def test_buyer_read_keeps_adjustment_indicator_bound_to_exact_line_and_tuple(
        self,
    ) -> None:
        """Buyer reads retain the adjustment indicator with its exact source tuple."""
        accepted = accept_bank_statement_evidence(
            self._command(
                self.changed_payload,
                "line-adjustment-indicator-lookup",
            ),
            posting.DATABASE_URL,
            self.case.policy.tenant_reference,
            artifact_store=self.store,
        ...
        self.assertEqual(
            ...
            self._structured_projection(self.changed_credit_debit_code),
        )
    ...
    def _structured_projection(
        self,
        second_line_later_credit_debit_code: str,
    ) -> list[dict[str, object]]:
        """Return the canonical projection with one isolated adjustment indicator."""
        ...
        line_details[1]["adjustments"] = [
            ...
            },
            {
                ...
                "currency_code": self.adjustment_currency_code,
                "credit_debit_code": second_line_later_credit_debit_code,
                "reason_code": "FEES",
    ...
    def _replace_second_line_later_adjustment_indicator(
        self,
        payload: bytes,
        old_credit_debit_code: str,
        new_credit_debit_code: str,
    ) -> bytes:
        """Replace only the later adjustment indicator inside the second source line."""
        ...
        old_block = (
            "<AdjstmntAmtAndRsn>\n"
            f'                        <Amt Ccy="{self.adjustment_currency_code}">{amount_text}</Amt>\n'
            f"                        <CdtDbtInd>{old_credit_debit_code}</CdtDbtInd>\n"
            "                        <Rsn>FEES</Rsn>\n"
            "                        <AddtlInf>Processing fee</AddtlInf>\n"
            "                      </AdjstmntAmtAndRsn>"
        )
        new_block = (
            "<AdjstmntAmtAndRsn>\n"
            f'                        <Amt Ccy="{self.adjustment_currency_code}">{amount_text}</Amt>\n'
            f"                        <CdtDbtInd>{new_credit_debit_code}</CdtDbtInd>\n"
            "                        <Rsn>FEES</Rsn>\n"
            "                        <AddtlInf>Processing fee</AddtlInf>\n"
            "                      </AdjstmntAmtAndRsn>"
        )
    ...
    def _command(self, payload: bytes, suffix: str) -> dict[str, object]:
        ...
        return {
            ...
            "ingestion_idempotency_key": (
                f"structured-line-adjustment-credit-debit-{suffix}-{uuid.uuid4().hex}"
            ),
