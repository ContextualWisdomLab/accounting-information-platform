...
Use PostgreSQL 18 and keep the migration owner, application runtime login and administrative / break-glass identities separate. Apply migrations in numeric order through `0015_reconciliation_policy_repair.sql` before starting the service. Existing databases that already ran `0014_reconciliation_candidate_allocation.sql` must apply the forward policy-repair migration before readiness is expected to pass. Do not run the application with a table-owner, superuser or `BYPASSRLS` login.
...
database/migrations/0015_reconciliation_policy_repair.sql
```
...
transactions to sixty seconds. Readiness sessions install their statement
timeout in PostgreSQL connection startup options before issuing any connected
command, cap every connected statement operation at five seconds, and preserve
a stricter configured timeout. Readiness uses a single-host database URL so
connection attempts cannot multiply that total budget. State-changing proposal, adjusting, reversal,
...
`GET /healthz` is a liveness probe: it proves only that the mounted process can answer. `GET /readyz` is a database-readiness probe: it verifies PostgreSQL 18 connectivity, an active database-controlled tenant binding, the required checked-in migration/schema contract through `0015_reconciliation_policy_repair.sql`, and the enabled exact table/trigger/function/event/column registrations for the checked-in behavior-defining accounting controls. These include journal book-scope and period guards, reversal temporal/finalization guards, posted-ledger and proposal/receipt immutability, finalized-journal extension guards, fiscal-period-open command immutability, soft-close evidence immutability, bank-statement source-evidence immutability, and reconciliation-run scope immutability. Each required trigger's zero-argument function must retain its canonical stored-function definition fingerprint; the two deferred database-owned journal-balance constraint triggers additionally require their canonical constraint/deferrable registrations and the canonical `assert_journal_balance()` fingerprint. Required constraints must retain their owning relation, type, validated/enforced and non-deferrable state, and canonical `pg_get_constraintdef()` definition; required indexes must retain their owning relation, valid/ready and uniqueness state, predicate, and canonical `pg_get_indexdef()` definition. Required triggers must be enabled, must not carry an unexpected `WHEN` predicate, and must preserve their exact unrestricted or `UPDATE OF` column scope before readiness returns `200 {"status":"ready"}`. Every tenant-scoped fact table must retain forced RLS, its exact public tenant-isolation policy, and the canonical tenant-binding function definition. Recorded canonical column metadata must remain an ordered prefix of each required table, so compatible additive tables and columns do not remove an instance from service while missing or altered required columns still fail closed. Its connection and complete statement operation are each capped at five seconds unless an operator configured a stricter timeout; the startup bound is active before the first connected readiness command, and readiness never accepts the privileged-session fallback used by existing accounting command paths. Both readiness responses send `Cache-Control: no-store` so an intermediary cannot reuse a stale readiness result. A failed readiness check returns `503` with operator-safe guidance and does not expose driver or connection details. Check migrations, runtime tenant provisioning, trigger enforcement, and database connectivity before retrying.

Readiness uses one total five-second deadline across connection establishment,
tenant binding, and all catalog statements; a multi-host database URL is
rejected because per-host connection timeouts could multiply that budget.
