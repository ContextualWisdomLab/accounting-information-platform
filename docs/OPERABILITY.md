...
Use PostgreSQL 18 and keep the migration owner, application runtime login and administrative / break-glass identities separate. Apply migrations in numeric order through `0012_bank_assignment_command_identity.sql` before starting the service. Do not run the application with a table-owner, superuser or `BYPASSRLS` login.
