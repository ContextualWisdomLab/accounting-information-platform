...
Use PostgreSQL 18 and keep the migration owner, application runtime login and administrative / break-glass identities separate. Apply migrations in the checked-in authority order through `0024_reconciliation_control_recording_time_authority.sql` before starting the service. Do not run the application with a table-owner, superuser or `BYPASSRLS` login.
...
Migration `0024_reconciliation_control_recording_time_authority.sql` makes `reconciliation_exception.recorded_at` and `reconciliation_evidence.recorded_at` database-owned by overwriting caller input with `clock_timestamp()` in `BEFORE INSERT` triggers while preserving separate business-valid `effective_at`. Because pre-0024 rows do not prove whether `recorded_at` came from the column default or from an explicit caller value, the upgrade first grants only `current_user` transaction-scoped all-tenant SELECT visibility on the two FORCE-RLS tables and fails closed with `reconciliation_recording_time_legacy_preflight` when either table is populated. The temporary policies are removed before durable guards are installed, and a failed migration rolls them back. A new run does not make retained legacy rows trustworthy; stop the upgrade and use a separately reviewed, auditable remediation/migration plan rather than deleting, rewriting, or grandfathering provenance that cannot be proven.
...
Migration `0024` imposes a separate system-time provenance preflight. Even an `open` exception or ordinary retained reconciliation-evidence row created before 0024 is not automatically admissible because the older schema allowed explicit `recorded_at`. If either table is populated when 0024 is attempted, keep the database at its pre-0024 state, preserve a restorable backup and exact row inventory, and obtain an explicit audited remediation/migration decision. Do not delete or rewrite immutable evidence merely to make the preflight pass, and do not claim that creating a new run fixes retained historical rows. Only a clean target store or a separately reviewed migration that can prove and preserve the original provenance may proceed to 0024.
