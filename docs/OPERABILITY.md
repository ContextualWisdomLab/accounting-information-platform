...
Migration `0024_reconciliation_control_recording_time_authority.sql` makes `reconciliation_exception.recorded_at` and `reconciliation_evidence.recorded_at` database-owned for new rows while preserving pre-0024 history exactly. The migration adds row-level `recording_time_authority_code`: existing rows receive `legacy_unverified`, their stored timestamps are not changed, and the constant historical marker does not pretend to prove how those timestamps were created. The migration then drops the marker defaults. `BEFORE INSERT` guards overwrite both caller-supplied `recorded_at` and authority markers with PostgreSQL `clock_timestamp()` plus `database_clock`; UPDATE guards make that pair immutable. A new maker-checker resolution command requires the exact exception and retained review artifact to carry `database_clock`, so `legacy_unverified` history remains auditable but cannot be promoted into new authority. This is a non-destructive forward migration: do not delete, rewrite, or relabel historical control evidence to make it trusted.

Migration `0025_reconciliation_lifecycle_recording_time_authority.sql` makes `reconciliation_run_transition_command.recorded_at` database-owned for every new lifecycle decision. PostgreSQL overwrites any caller-shaped system time with `clock_timestamp()` and rejects `effective_at > recorded_at` using the stable `reconciliation_lifecycle_future_time` marker before a future-effective decision can become current reconciliation authority. `effective_at` remains business-valid time. This migration does not post/reverse journals, close fiscal periods, or alter accounting policy.
...
Migration `0024` has a different upgrade contract. Pre-0024 exception/evidence rows are preserved with their original timestamps and receive `recording_time_authority_code = 'legacy_unverified'`; do not rewrite those timestamps or manually promote the marker. After 0024, newly inserted exception/review rows receive `database_clock` only from PostgreSQL together with a database-owned `recorded_at`. A new exception-resolution command fails closed if either exact source row is still `legacy_unverified`. When an unresolved historical exception requires a new authoritative decision, retain the old run for audit and create a new reconciliation run plus review evidence after 0024 rather than mutating the old row. Backup/restore rehearsal must verify that both marker values and their associated timestamps survive unchanged.

Migration `0025` governs new lifecycle decisions rather than rewriting prior transition history. After 0025, do not supply or depend on a caller-shaped `recorded_at`; the database chooses the system time and rejects a business-valid `effective_at` later than that instant. A rejected future-effective transition must leave transition command, run status and outbox authority evidence rolled back together. Upgrade and recovery rehearsal must verify this fail-closed behavior before the lifecycle stack is released.
