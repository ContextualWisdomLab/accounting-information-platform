...
`GET /healthz` is a liveness probe: it proves only that the mounted process can answer. `GET /readyz` is a database-readiness probe: it verifies PostgreSQL 18 connectivity, an active database-controlled tenant binding, the complete checked-in migration/schema contract through `0014_reconciliation_candidate_allocation.sql`, and the enabled exact relation/function registrations for every database-owned guard trigger through migration `0013_reconciliation_run_exception_evidence.sql`. This includes each deferred journal-balance trigger's canonical `assert_journal_balance()` definition fingerprint, unrestricted events without `WHEN` predicates or `UPDATE OF` column filters, and the exact `UPDATE OF` columns on scope/evidence guards, before returning `200 {"status":"ready"}`. Its connection attempt is capped at five seconds unless an operator configured a stricter timeout; it never accepts the privileged-session fallback used by existing accounting command paths. Both readiness responses send `Cache-Control: no-store` so an intermediary cannot reuse a stale readiness result. A failed readiness check returns `503` with operator-safe guidance and does not expose driver or connection details. Check migrations, runtime tenant provisioning, trigger enforcement, and database connectivity before retrying.
