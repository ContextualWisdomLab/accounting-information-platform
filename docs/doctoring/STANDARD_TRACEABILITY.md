...
| IAS 10 | Soft-close rejects ordinary posts and allows append-only reversing adjustments before hard-close snapshots and locks the period. Auditors later read those durable hard-close receipts from stored snapshots, not reconstructed soft-close history | Two-step `POST /period-closes`, HTTP period-close list, ADR 0023, and ADR 0030 |
