...
| AICPA Trust Services Criteria (SOC 2) | Auditors read an append-only history of posted, reversed, and closed facts from existing `outbox_event` rows, including already-published rows, without marking publish. Controllers also list stored `journal_reversal` lineage over HTTP without SQL | HTTP audit-event history, HTTP journal-reversal list, ADR 0027, and ADR 0029 |
