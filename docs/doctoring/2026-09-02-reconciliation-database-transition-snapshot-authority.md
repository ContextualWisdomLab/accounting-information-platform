...
The base `0019_reconciliation_run_command_evidence.sql` remains an unreleased stacked migration. The authority overlay is deliberately named `0019_reconciliation_run_database_snapshot_authority.sql` rather than consuming child migration number `0020`. `migration_install.apply_foundation_migration()` first applies the established base chain and then this bounded overlay; it fails closed if the overlay is missing or PostgreSQL rejects it. The compatibility assignment to `persistence.apply_foundation_migration` prevents older integration fixtures/operator imports from silently stopping after the caller-trusting base definition.
