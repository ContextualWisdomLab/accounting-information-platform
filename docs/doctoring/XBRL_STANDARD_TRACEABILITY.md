...
| RFC 9110 HTTP Semantics §§4.2, 4.2.4 | `http` and `https` URIs identify an origin through the authority host; HTTP(S) senders must not generate URI `userinfo`, and recipients of untrusted HTTP(S) references should treat its presence as an error | Require a non-empty parsed hostname and reject any parsed HTTP(S) username/userinfo for caller-supplied namespace/schema/entity identifiers after generic authority and port parsing. Credentials or principal identity belong to a purpose-limited transport/secret boundary, never the immutable report/profile identifier | `financial_reporting/primitives.py`; `tests/test_financial_reporting_uri_rfc3986_red.py::FinancialReportingUriRfc3986Tests.test_absolute_uri_rejects_http_authority_without_host`; `tests/test_financial_reporting_uri_rfc3986_red.py::FinancialReportingUriRfc3986Tests.test_absolute_uri_rejects_http_userinfo` | **Implemented at the proposal input boundary.** This rejects hostless forms and `https://user@host/...` / `https://user:password@host/...`; it does not dereference a URL or establish network/taxonomy trust |
| RFC 8141 Uniform Resource Names | A URN assigned name has the structural form `urn:NID:NSS`; the namespace identifier has bounded alphanumeric/hyphen syntax and the namespace-specific string is non-empty. Generic URI authority syntax is not the URN assigned-name form | Fail closed on authority-form URNs, a missing or syntactically invalid NID, or an empty NSS while preserving valid assigned-name identifiers | `financial_reporting/primitives.py`; `tests/test_financial_reporting_uri_rfc3986_red.py::FinancialReportingUriRfc3986Tests.test_absolute_uri_rejects_urn_authority_and_missing_nss` | **Bounded structural validation implemented.** This does not claim complete RFC 8141 component validation, namespace registration, identifier resolution, dereferencing, or taxonomy trust |
...
Saint-Andre, P., & Klensin, J. (2017). *Uniform Resource Names (URNs)* (RFC 8141). RFC Editor. https://doi.org/10.17487/RFC8141
