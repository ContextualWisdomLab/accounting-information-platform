...
| RFC 9110 HTTP Semantics §4.2 | `http` and `https` URIs identify an origin through the authority host; senders must not generate, and recipients must reject, an HTTP(S) URI with an empty host | Require a non-empty parsed hostname for every HTTP(S) namespace/schema/entity URI after generic authority and port parsing | `financial_reporting/primitives.py`; `tests/test_financial_reporting_uri_rfc3986_red.py::FinancialReportingUriRfc3986Tests.test_absolute_uri_rejects_http_authority_without_host` | **Implemented at the proposal input boundary.** This rejects hostless forms such as `https://:443/...` and `https://user@/...`; it does not dereference a URL or establish network/taxonomy trust |
