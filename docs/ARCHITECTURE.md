...
36. `database/migrations/0035_trial_balance_snapshot_hard_close_pair.sql` — defers snapshot-side validation to commit and rejects retained trial-balance evidence whose exact book-period control does not end `hard_closed`.
37. `database/migrations/0036_hard_close_trial_balance_snapshot_pair.sql` — defers the inverse status-side validation to commit and rejects a transition to `hard_closed` without the exact retained trial-balance snapshot.
