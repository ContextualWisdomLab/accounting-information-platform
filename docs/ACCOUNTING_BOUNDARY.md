...
A proposal may name `accounts_receivable`, `usage_revenue`, `contract_liability`, `tax_payable`, `cash_clearing`, or `provider_fee_expense`. Accounting maps those roles to chart accounts under an effective policy version. Source systems cannot bypass this mapping by sending chart-account IDs.
