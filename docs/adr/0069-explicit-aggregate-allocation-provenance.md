...
- This ADR owns journal-side aggregate provenance. ADR 0071 subsequently supersedes the earlier scalar statement-pair contract and requires exact repository-owned `StatementEntryEvidence` on the statement side.
...
- Distinct-statement identity, exact Decimal conservation, tenant/run scope, and immutable allocation output remain part of the aggregate contract; ADR 0071 later strengthens statement-population provenance from scalar pairs to exact `StatementEntryEvidence` without weakening this journal-side decision.
...
ADR 0071 begins at RED `d38e13a9cac9683102a1d1bf70b66c6ad177e49f` on #161 exact `377f39cd798e3351993aee5b785d1f2a7f31e1fb` and strengthens the orthogonal statement-side source boundary. Its causal repair `b3cb6b842297786d295ca54570a96f37762afb3f` requires exact `StatementEntryEvidence`, revalidates statement and journal currency/direction at use, and rejects cross-currency or opposite-direction aggregate populations before conservation. This does not replace or weaken the journal-side provenance controls defined here.
...
This still does not prove that a supplied `BookJournalEvidence` came from the current PostgreSQL-owned snapshot. Persistence, approval, lifecycle, and close-package owners remain responsible for binding reviewed allocation evidence to authoritative database state, lock ordering, tenant scope, and recovery controls. This ADR only closes the in-memory journal-side proposal boundary; ADR 0071 closes the corresponding statement-side provenance weakness.

## Follow-up

PR #37 remains the canonical single writer for shared `CHANGELOG.md`, `docs/doctoring/STANDARD_TRACEABILITY.md`, and `docs/product-technical-gap-baseline.md`. After this decision and its ADR 0071 successor reach protected integration through the Bank Reconciliation owner path, #37 must rebuild those records from the exact protected tree and record the progression from synthetic aggregate provenance to explicit scalars, exact `BookJournalEvidence`, and exact `StatementEntryEvidence` admission plus runtime revalidation. Until then both ADRs stay Proposed and mutable-branch evidence must not be promoted to integrated product truth.
