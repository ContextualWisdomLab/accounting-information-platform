...
The in-memory `PostingLedger` remains the executable reference oracle for exact command replay, and the PostgreSQL adapter preserves the same command-key plus immutable-hash semantics on durable rows. Release evidence must prove that durable contract on one unchanged exact integrated protected head; a passing cache-only replay or predecessor-head test is not sufficient release evidence.
