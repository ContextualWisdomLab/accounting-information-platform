...
When a statement entry reconciles against several journal candidates, the split must be planned in exact `Decimal` and the allocations must sum exactly to the statement entry amount; the allocation module allocates each candidate's full amount and fails closed if the candidate total differs from the statement amount. When several statement entries reconcile to one journal total, the aggregate allocations must conserve the exact total on both sides; a book-side total that differs from the statement-side sum fails closed. Each `ReconciliationAllocation` is immutable, tenant- and run-scoped, carries the statement and journal reference plus currency, and rejects non-exact or non-positive money. Allocation plans are still evidence: they never post, reverse, approve, or adjust a journal. Migration `0014_reconciliation_match_allocation.sql` persists the proposed match and normalized statement/journal allocation rows with forced tenant isolation, same-scope guards, append-only mutation guards, and deferred exact conservation; approval and double-consumption concurrency remain later bounded controls.
