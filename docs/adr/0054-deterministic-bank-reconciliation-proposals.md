...
A preceding-run delta is permitted only when the current and preceding evidence carry the same immutable tenant, legal entity, accounting book, bank-account assignment, and currency scope. Same-currency evidence from another book or bank account is not comparable and fails closed rather than producing plausible historical deltas. The preceding bridge retains its own run and population identities; the scope comparison proves comparability without rewriting either immutable population.
