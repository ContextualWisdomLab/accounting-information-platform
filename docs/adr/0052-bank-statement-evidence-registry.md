...
The relational database stores hashes, message metadata, locators, and normalized facts. The original artifact belongs in a host-owned immutable evidence store; HTTP binds a process-lifetime store, and a library caller must supply one. Reuse of an ingestion idempotency key with a changed artifact fails closed. The same source bytes produce one canonical statement population and serialize on that digest so concurrent keys cannot race the unique constraint into an unhandled error. The same statement identity with unchanged normalized entries is duplicate delivery and replays the first artifact; changed material entry evidence fails closed until an explicit correction contract exists.
