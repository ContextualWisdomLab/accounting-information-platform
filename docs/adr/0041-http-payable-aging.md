...
Aging reuses the receivable FIFO engine with the side inverted. Credit lines are payable increases (tax assessed) and age from stored `accounting_date`. Debit lines (credit tax-unwind, a Billing issued-invoice void that debits `tax_payable` on `{tenant}:issued_invoice_void:{issued_invoice_void_id}:{void.source_payload_hash}:v{issued_invoice_void_contract_version}`, and reversing payments) consume the oldest open credit first using exact decimal arithmetic. A taxed issued-invoice void drops `total_outstanding_amount` by that exact tax credit (fixture 2500 → 0) and the total still equals `GET /account-balances` net (`credit_amount` − `debit_amount`) for 210100. An untaxed void has no `tax_payable` line and leaves payable aging at zero. `GET /payable-agings` defaults to catalog `tax_payable` 210100; `chart_account_code=210200` stays 422. Adjusting journals that touch the payable account are included. The AIS period-closing journal is excluded and is not used as an assessment date. After hard-close, AIS still ages from live journals through period end; it does not invent dates from the trial-balance snapshot. `total_outstanding_amount` is the sum of the four unsigned buckets and equals `GET /account-balances` net (`credit_amount` − `debit_amount`) for that payable account and period, including the stored snapshot net after hard-close. AIS does not add a void `account_role_code`, require Billing `journal_entry_id`, or emit invoice or collection status.
