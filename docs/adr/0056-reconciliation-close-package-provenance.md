...
The authoritative reconciliation run already exists as durable PostgreSQL evidence. `accounting_core.reconciliation_run.knowledge_cutoff_at` and the evaluated tenant/entity/book/bank-assignment/currency scope are immutable after creation, while the `reconciliation_run_command` table (`accounting_core.reconciliation_run_command`) binds the accepted run command to its database-owned canonical digest, source payload hash/reference, and retained bank-statement record. A close package that trusted caller-supplied copies of those values could misstate the run's knowledge boundary, relabel the evaluated accounting scope, or archive an unrelated source artifact while remaining internally self-consistent. Authority-bearing package construction therefore has to re-read and bind those values from PostgreSQL.

`accounting_core.reconciliation_exception` is also authoritative run state. A caller-shaped close projection can otherwise report zero exceptions even while the same tenant/run still has an `open` exception row. Because a close package is eligible only for a clean close-review candidate, construction must lock and re-read the complete run exception population and fail closed when the database-owned open-exception count disagrees with the projection.
...
A projection is package-eligible only when `suitable_for_period_close_review` is true, the exception count and exception-reference population are empty, and the unexplained difference is exact `Decimal("0")`. The authority-bearing builder additionally requires the database-owned open-exception population for the tenant/run to be empty by requiring its count to equal the projection's zero exception count. Bridge and package arithmetic use dedicated exact-decimal contexts rather than caller ambient precision or exponent bounds. Eligibility is evidence suitability only; it does not authorize reconciliation approval, period close, posting, reversal, or accounting-policy change.
...
- the database-owned reconciliation-run tenant/entity/book/bank-assignment/currency scope, resolved from the persisted run and its referenced accounting master rows, which must exactly equal the public projection scope;
- the database-owned run exception population under a shared lock, with every current `open` exception counted and required to agree with the projection before package construction can proceed;
...
`build_reconciliation_close_package()` is the authority-bearing construction path. It requires a tenant-bound PostgreSQL session and does not treat caller-shaped run, accounting scope, exception state, statement-artifact, match-state, or snapshot-tenant evidence as authoritative.
...
For reconciliation exceptions, the builder reads the complete `accounting_core.reconciliation_exception` population for the same tenant/run under a shared lock and derives the current unresolved population from rows whose `resolution_status_code` is `open`. Its count must equal the projection's `exception_count`. Because `_validate_projection()` permits an authority-built close package only when that count is zero and the exception-reference population is empty, any open database exception blocks package creation even if the caller submits a clean projection. `resolved` and `superseded` historical exception rows remain durable evidence but do not count as current unresolved blockers.
...
Controllers and acquirers receive deterministic close evidence whose persisted accounting scope, source artifact, immutable run boundary, unresolved-exception state, complete approved-match population, exact tenant snapshot identity binding, and exact book-to-bank facts can be rehashed and compared. A caller cannot relabel an otherwise valid package with another legal entity, accounting book, bank assignment, currency, run cutoff or command digest; hide an open database exception behind a zero-exception projection; substitute an unrelated statement artifact; provide a forged internal tenant identity binding; repurpose a superseded approval as currently approved evidence; silently swap statement/book populations; or hide an internally inconsistent bridge behind a zero unexplained-difference field.

Construction fails closed when the database-owned run or source-artifact population is absent or ambiguous; persisted tenant/entity/book/bank-assignment/currency scope differs from the projection; the database-owned open-exception count differs from the projection; run command, statement record, and retained artifact hashes/references disagree; caller run/cutoff/artifact evidence differs from PostgreSQL; current match state is not approved; approval evidence is missing, rejected, duplicated, incomplete, outside the projection scope, or cannot be reproduced with the database-owned tenant identity; required population evidence is absent or mismatched; exact allocation/capacity/bridge invariants fail; or identifiers, timestamps, digests, and money values are non-canonical.
