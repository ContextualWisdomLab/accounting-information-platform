...
Third, `guard_reversal_temporal_order` / `journal_reversal_temporal_guard` resolves both tenant-scoped journal dates before reversal lineage is recorded and rejects a reversal whose accounting date precedes the original journal accounting date. This is a database-owned cutoff invariant: direct SQL cannot make a corrective fact appear in a trial-balance or reporting cutoff before the fact it corrects, even if an application validation path is bypassed.

Fourth, database mutation guards reject `UPDATE` or `DELETE` of posted journal headers, lines, source references, reversal lineage, posting receipts, and proposal source evidence. After a `posting_receipt` identifies a journal as finalized, later `INSERT` into that journal's `journal_entry_line` or `journal_source_reference` population also fails closed with `ledger_immutable`. The initial posting transaction remains valid because it writes lines and source references before issuing the authoritative receipt.

Fifth, `guard_reversal_lineage_insert` / `journal_reversal_finalization_guard` constrains reversal lineage to the atomic reversal transaction. The original journal must already have an authoritative posting receipt, while the candidate reversal journal must not yet have a receipt when the lineage row is inserted. The normal reversal path therefore remains valid—append reversal journal and lines, record lineage, then issue its receipt—but a journal that was already finalized for another purpose cannot later be repurposed as a reversal by inserting lineage after receipt issuance.

## Consequences

Historical audit evidence remains intact. APIs, database permissions, migrations, and runtime roles must not expose an ungoverned journal mutation, temporal-order bypass, finalization-lineage bypass, or close-bypass path. Reporting reconstructs effects from the complete population. A later command that reuses a posted `proposal_id` with a new idempotency key must reverse the existing journal, then post a replacement. Database migration tests and real PostgreSQL integration tests must prove commit-boundary balance, reversal temporal ordering, finalized-population and lineage immutability, and soft-close authorization before this decision is considered release-ready.
