...
`PostingLedger.reverse` and `PostgresPostingLedger.reverse` also fail closed when a journal already occupies `{journal_reference}:reversal`. They do not overwrite that posted journal. If the occupant is the existing reversing journal for that original, the same reversal request still replays. Commercial `proposal_id` is a hyphenated UUID (the published Billing identifier charset) so a proposal cannot construct that reversal key with a `:` or `:reversal` suffix.
