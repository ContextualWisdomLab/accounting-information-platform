...
Rejected receipts that have resolved catalog foreign keys are persisted on `accounting_integration.home_tax_submission`. The row stores `as_of_date`, `closing_amount`, and `register_payload_hash`; it does not store raw register JSON, NTS payloads, or secrets. `GET /home-tax-submissions?legal_entity_reference=&book_reference=&fiscal_period_reference=` lists those receipts. An empty scope is `home_tax_submissions` [], not 404. Reconstruct `vat_period_register` as `as_of_date` plus `closing_amount`. `POST /vat-period-registers` remains 405. Billing does not receive a VAT or HomeTax endpoint. AIS does not invent `tax_invoice` or `input_vat` catalog roles.
