...
A database-owned temporal non-overlap constraint or equivalent invariant is preferred over application-only validation because concurrent catalog writes must not create ambiguous authority and later validity updates must not bypass the same rule. A partial unique index limited to `valid_to IS NULL` is insufficient by itself: it prevents duplicate open-ended rows but still permits finite rows whose validity overlaps an open-ended or another finite row. Resolver cardinality checks remain defense in depth and provide a stable fail-closed error if legacy/corrupt state is encountered.

## Alternatives rejected

Making `book_role_code` part of the public identity is rejected because ADR 0022 already exposes `book_name` as the durable reference and callers should not have to infer an accounting role to disambiguate one identifier. Selecting the first match is rejected because result identity would depend on query order. A current-row-only uniqueness rule is rejected because it does not protect effective-time reads from overlapping historical/future intervals. An insert-only validation path is rejected because a lawful historical interval can later be edited into overlap. A one-sided update trigger is rejected because changing either `valid_from` or `valid_to` can create overlap. Making `book_name` globally unique for all history is rejected because it would prevent lawful non-overlapping effective-dated history. Closed-interval semantics are rejected because they would make a row ending at `t` conflict with its successor beginning at `t`, despite no instant having two effective Entities under the repository's established `valid_from <= t` / `valid_to > t` convention. Rewriting posted journals or retained evidence to a newly chosen book is rejected because posted accounting facts are immutable.
