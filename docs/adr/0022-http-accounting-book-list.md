...
The database gap is not limited to inserts. `valid_from` / `valid_to` are effective-time state, so an initially lawful pair of adjacent or disjoint rows can become ambiguous through a later validity update unless the same database-owned invariant is enforced on mutation. An insert-only trigger or application preflight is therefore insufficient even before concurrency is considered.
...
- enforce non-overlapping effective-time identity for `(tenant_account_id, legal_entity_id, book_name)` in PostgreSQL for both INSERT and effective-time UPDATE, across open-ended and finite validity intervals;
