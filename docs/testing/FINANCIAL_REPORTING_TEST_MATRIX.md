...
| URI syntax / HTTP origin boundary | Taxonomy namespace/schema/entity URIs reject unescaped whitespace, malformed percent escapes, raw backslashes, non-numeric HTTP(S) authority ports, and HTTP(S) authorities with no parsed host while preserving valid percent encoding | `tests/test_financial_reporting_uri_rfc3986_red.py` |
