...
| URI syntax / HTTP origin / URN assigned-name boundary | Taxonomy namespace/schema/entity URIs reject unescaped whitespace, malformed percent escapes, raw backslashes, non-numeric HTTP(S) authority ports, HTTP(S) authorities with no parsed host, any HTTP(S) userinfo including username/password material, URN authority syntax, missing or invalid URN namespace identifiers, and empty URN namespace-specific strings while preserving valid percent encoding and a valid `urn:NID:NSS` assigned name | `tests/test_financial_reporting_uri_rfc3986_red.py` |
