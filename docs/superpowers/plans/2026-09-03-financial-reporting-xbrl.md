...
**Architecture:** Add one stateless `financial_reporting` package downstream of `load_financial_statement_package`. Exact decimal facts and source paths form a canonical artifact. XBRL serialization accepts an immutable external taxonomy profile and performs no taxonomy fetch or accounting calculation.
...
5. Assert invalid profile identifiers, prefixes, namespaces, schema references, digests, duplicate mappings, missing fact mappings, period mismatches, and artifact tampering fail closed.
6. Open a draft pull request so the missing production package is observed as the intended RED state before implementation.
...
5. Verify profit-or-loss, financial-position, changes-in-equity, and cash-flow invariants and cross-statement ties.
...
4. Rebuild and compare the report artifact from its embedded source evidence before export.
5. Map canonical facts through the supplied profile and reject absent facts or mapping/fact period-type disagreement.
6. Serialize mapped monetary facts with explicit context, unit, and decimal precision.
7. Return the XML instance, media type, profile and taxonomy provenance, source report hash, and instance hash.
...
1. Preserve the test-first commit that demonstrates the intended missing-API RED state.
2. Run focused isolated verification after each causal implementation change.
