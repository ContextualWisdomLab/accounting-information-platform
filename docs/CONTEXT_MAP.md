...
- `psycopg` is the one currently approved third-party import root because it is the PostgreSQL infrastructure driver used by repository-owned persistence and reconciliation transaction boundaries. This exception does not make `psycopg` a domain dependency, Shared Kernel, foreign application implementation, or accounting decision authority; every other non-stdlib absolute import remains deny-by-default until explicitly reviewed and documented.
...
| `src/accounting_information_platform/reconciliation_lifecycle.py` | `reconciliation_run_control` | Session-lease/advisory-lock and fresh `REPEATABLE READ` orchestration only; review decisions, journal posting and period close remain outside this owner | aligned | Keep lifecycle coordination in run control without acquiring reconciliation-review, posting or close authority |
...
4. Domain calculations must remain executable without HTTP/framework/provider DTOs. The explicitly reviewed `psycopg` import root is a PostgreSQL infrastructure adapter exception only; it does not authorize persistence types or driver objects to become domain entities or own accounting invariants.
