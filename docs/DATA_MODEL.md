...
- `bank_statement_balance`: one immutable numeric balance fact per statement sequence, retaining optional balance type, exact amount, ISO currency, credit/debit direction, source locator, and source hash. Migration `0018_bank_statement_balance_evidence.sql` preserves the values that migration `0011_bank_statement_evidence.sql` previously represented only by opening/closing hashes; missing numeric facts remain unavailable for a bridge rather than being inferred.
