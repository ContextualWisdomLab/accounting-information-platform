...
- `ReconciliationClosePackage` is a read-only schema-versioned manifest over those rows, not another database authority. Its close-review input requires each match decision to carry a canonical durable match identity and carries that identity with its statement/journal/amount facts in a `ReconciliationReviewedMatch` record bound to the matching deterministic decision; the complete structured records remain in the projection and package export. The package then binds that reviewed population to approved decisions, exact tenant/run scope, PostgreSQL snapshot digests, durable approval-evidence references, immutable run cutoff, and source-population references. Incomplete, rejected, unrelated, or source-mismatched approval evidence fails closed before export.
