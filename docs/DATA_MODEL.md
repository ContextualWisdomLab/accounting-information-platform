...
- `bank_statement_balance`: one immutable numeric balance fact per statement sequence, retaining optional balance type, its `balance_type_source_code` discriminator (`cd` or `prtry`), exact amount, ISO currency, credit/debit direction, typed `balance_effective_at`, source locator, and source hash. Migration `0018_bank_statement_balance_evidence.sql` preserves the values that migration `0011_bank_statement_evidence.sql` previously represented only by opening/closing hashes; `balance_effective_at` is separate from statement period and `recorded_at`, and missing numeric facts remain unavailable for a bridge rather than being inferred.
...
Migrations `0013_reconciliation_run_exception_evidence.sql` through `0019_reconciliation_run_command_evidence.sql` persist deterministic reconciliation as accounting control evidence without granting posting, reversal, close, or accounting-policy authority.
...
- `reconciliation_command_identity`: database-owned tenant/idempotency-key registry shared by reconciliation run-opening and run-reconciliation command families. Its primary key is the cross-family concurrency boundary; command-family ownership is immutable, forced through tenant RLS, and contains no journal, bank-balance, or close amount facts.
- `reconciliation_run_command`: one immutable tenant/run/statement command identity with tenant-scoped idempotency key, canonical command hash, immutable source-payload hash/reference, and the evidence recorded when an `evaluating` run opens. New runs select only source facts recorded no later than their knowledge cutoff; a deferred database provenance guard requires exactly one command row and binds its statement to the run assignment's bank account. A BEFORE INSERT trigger reserves the same tenant/key in `reconciliation_command_identity`. Exact retries replay the run; changed command evidence or cross-family reuse under the same key fails closed. It does not select a chart account or authorize matching, approval, close, or posting.
- `reconciliation_run_transition_command`: immutable evidence for the supported `evaluating`/`review_required` → `reconciled` lifecycle edge. It binds the database-derived review/source snapshot, statement/book population identities, actor, purpose, effective time, target status, and database-assigned command hash. Its BEFORE INSERT trigger reserves the lifecycle idempotency key in the same `reconciliation_command_identity` namespace, so opening and completion commands cannot concurrently commit different meanings under one tenant/key. The command, status update, and `reconciliation_run_reconciled` outbox event commit atomically.
- `ReconciliationClosePackage` is a read-only schema-versioned manifest over those rows, not another database authority. Its close-review input requires each match decision to carry a canonical durable match identity and carries the candidate facts plus complete deterministically ordered statement/journal allocation populations in a `ReconciliationReviewedMatch` record bound to the matching durable decision. The package recomputes migration 0016's version-1 snapshot digest from those facts and binds it to the database-owned approval row, including its immutable command source-payload hash/reference. Split and aggregate matches and zero-activity runs are supported; incomplete, rejected, unrelated, substituted, or source-mismatched evidence fails closed before export.
...
- Command idempotency is tenant-scoped and tied to immutable source/command evidence so exact retries replay and changed evidence fails closed. Reconciliation run-opening and lifecycle commands additionally share one database-owned tenant/key identity registry so cross-family uniqueness survives concurrent transactions.
